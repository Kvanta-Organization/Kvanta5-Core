...
Kvanta5 Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/kvanta5-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `KVANTA5_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Kvanta5 Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Kvanta5 Core version 0.18.0.
...
- Testing or debugging of implementations of the Kvanta5 P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Kvanta5 Core. Kvanta5 Core logs debug messages
...
- OpenSSL is no longer used by Kvanta5 Core.  (#17265)

- BIP70 support has been fully removed from Kvanta5 Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/kvanta5/kvanta5/issues/16599),
  [PR #16702](https://github.com/kvanta5/kvanta5/pull/16702), and the
  `kvanta5d help` for more information.  This option is experimental and
...
- All custom settings configured when Kvanta5 Core starts are now
...
- The "Start Kvanta5 Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Kvanta5 Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Kvanta5 Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/kvanta5-core/HWI) for
  signing.  Future versions of Kvanta5 Core should support a GUI option
...
- Kvanta5 Core's automatically-generated source code documentation is
...
- The `kvanta5-cli` utility used with the `-getinfo` parameter now
...
- The `kvanta5-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `kvanta5strings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_kvanta5-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_kvanta5` binary (hebasto)
...
- #18702 Fix ASLR for kvanta5-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate kvanta5 schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest kvanta5.conf (sdaftuar)
...
- #18022 Fix appveyor `test_kvanta5` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (kvanta5-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone kvanta5-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Kvanta5 Core files (hebasto)
...
- #17119 Fix broken kvanta5-cli examples (andrewtoth)
...
- #17503 Remove bitness from kvanta5-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to kvanta5-wallet (hel-o)
...
- #18341 Replace remaining literal KV5 with `CURRENCY_UNIT` (domob1812)
...
- #16115 On kvanta5d startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from kvanta5-cli -getinfo (malevolent)
