Kvanta5 Core version 0.11.0 is now available from:

  <https://bitcoin.org/bin/kvanta5-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer KV5/kB fee to be
...
For example, add the following to `kvanta5.conf`:
...
related to the blockchain in the kvanta5 system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Kvanta5 Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Kvanta5 Core will 
...
For an example script see [kvanta5-submittx](https://github.com/laanwj/kvanta5-submittx).
...
- #5334 `cf87536` libkvanta5consensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new kvanta5-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only Kvanta5AmountField
- #5753 `0f44672` Add kvanta5 logo to about screen
...
- #5809 `46bfbe7` Add kvanta5-cli man page
...
- #5849 `41259ca` contrib/init/kvanta5d.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Kvanta5-Qt.app to Kvanta5-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for kvanta5-tx)
