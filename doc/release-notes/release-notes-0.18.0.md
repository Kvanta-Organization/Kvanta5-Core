Kvanta5 Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/kvanta5-core-0.18.0/>
...
`/Applications/Kvanta5-Qt` (on Mac) or `kvanta5d`/`kvanta5-qt` (on
Linux).
...
recommended to use Kvanta5 Core on unsupported systems.

Kvanta5 Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Kvanta5 Core does not yet change appearance when
...
`contrib/init/kvanta5d.service`, it has been changed to use
`/var/lib/kvanta5d` as the data directory instead of
`~kvanta5/.kvanta5`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/kvanta5d` will
exist has enough space (check using `df -h /var/lib/kvanta5d`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/kvanta5/kvanta5/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Kvanta5 Core
...
The systemd init file (`contrib/init/kvanta5d.service`) has been changed
to use `/var/lib/kvanta5d` as the data directory instead of
`~kvanta5/.kvanta5`. This change makes Kvanta5 Core more consistent with
...
`datadir` option in `/etc/kvanta5/kvanta5.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/kvanta5/kvanta5.conf`.


Documentation
-------------

- A new short [document](https://github.com/kvanta5/kvanta5/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/kvanta5/kvanta5/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/kvanta5/kvanta5/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/kvanta5/kvanta5/blob/master/doc/kvanta5-conf.md)
  about the `kvanta5.conf` file describes how to use it to configure
  Kvanta5 Core.

- A new document introduces Kvanta5 Core's BIP174 [Partially-Signed
  Kvanta5 Transactions
  (PSBT)](https://github.com/kvanta5/kvanta5/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/kvanta5/kvanta5/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Kvanta5-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Kvanta5 Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/kvanta5/kvanta5/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/kvanta5/kvanta5/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Kvanta5 Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Kvanta5 Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/kvanta5/kvanta5/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/kvanta5/kvanta5/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart kvanta5d with the `-deprecatedrpc=generate`
...
- A new `kvanta5-wallet` tool is now distributed alongside Kvanta5
...
This section describes planned changes to Kvanta5 Core that may affect
other Kvanta5 software and services.

- Since version 0.16.0, Kvanta5 Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Kvanta5 Core 0.20 (expected about
  a year after 0.18), Kvanta5 Core will default to native segwit
  ...
  to bech32 addresses, and if the Kvanta5 Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Kvanta5 Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Kvanta5 Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Kvanta5 Core
...
- A sub-project of Kvanta5 Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Kvanta5 Core.  See their [project
  page](https://github.com/kvanta5-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Kvanta5 Core's own implementation, although entropy
  gathered by Kvanta5 Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Kvanta5 Core a
...
- On macOS, Kvanta5 Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `kvanta5_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libkvanta5_server.a` (jnewbery)
...
- #13926 [Tools] kvanta5-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like kvanta5d (MarcoFalke)
...
- #14521 qt, docs: Fix `kvanta5-qt -version` output formatting (hebasto)
...
- #11625 Add Kvanta5Application & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libkvanta5_server` (laanwj)
...
- #15154 Configure: kvanta5-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/kvanta5-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Kvanta5 Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libkvanta5_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in kvanta5-util-test.py (isghe)
...
- #12775 Integration of property based testing into Kvanta5 Core (Christewart)
...
- #12246 Bugfix: Only run kvanta5-tx tests when kvanta5-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_kvanta5` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_kvanta5` (MarcoFalke)
...
- #14186 kvanta5-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_kvanta5.conf` message on startup if conf file exists (leishman)
...
- #12255 Update kvanta5.service to conform to init.md (dongcarl)
...
- #14497 Add doc/kvanta5-conf.md (hebasto)
...
- #15354 Add missing `kvanta5-wallet` tool manpages (MarcoFalke)
...
- #15765 correct kvanta5consensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support kvanta5 folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/kvanta5/).
