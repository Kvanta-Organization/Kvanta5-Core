Kvanta5 Core version 0.13.1 is now available from:

  <https://bitcoin.org/bin/kvanta5-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Kvanta5 wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Kvanta5.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Kvanta5 block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of kvanta5s they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of kvanta5s being spent is exactly
...
- **Increased security for multisig:** Kvanta5 addresses (both P2PKH addresses
  ...
  believed to be provided by Kvanta5's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Kvanta5 paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Kvanta5's current ledger (called the UTXO set),
  ...
  note that Kvanta5 Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Kvanta5 users to individually opt-in to almost any change in the Kvanta5
  Script language when those users receive new transactions.  Features
  currently being researched by Kvanta5 Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Kvanta5 community members are studying several other improvements
...
[BIP141]: https://github.com/kvanta5/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/kvanta5/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/kvanta5/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/kvanta5/bips/blob/master/bip-0145.mediawiki
...
Since Kvanta5 Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/kvanta5/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in kvanta5consensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.kvanta5stats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libkvanta5consensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_kvanta5 by itself (luke-jr)
...
- #8829 `2a8bca4` Add kvanta5-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` kvanta5-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_kvanta5ds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name kvanta5.qrc  (kvanta5sSG)
...
- #8817 `bcf3806` update kvanta5-tx to output witness data (jnewbery)
