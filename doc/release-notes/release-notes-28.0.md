Kvanta5 Core version 28.0 is now available from:

  <https://bitcoincore.org/bin/kvanta5-core-28.0/>
...
installer (on Windows) or just copy over `/Applications/Kvanta5-Qt` (on macOS)
...
Running Kvanta5 Core binaries on macOS requires self signing.
```
cd /path/to/kvanta5-28.0/bin
xattr -d com.apple.quarantine kvanta5-cli kvanta5-qt kvanta5-tx kvanta5-util kvanta5-wallet kvanta5d test_kvanta5
codesign -s - kvanta5-cli kvanta5-qt kvanta5-tx kvanta5-util kvanta5-wallet kvanta5d test_kvanta5
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Kvanta5
...
Support for Testnet4 as specified in [BIP94](https://github.com/kvanta5/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Kvanta5`
to `C:\Users\Username\AppData\Local\Kvanta5`. Kvanta5 Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/kvanta5/kvanta5/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libkvanta5consensus Removal
---------------------------

The libkvanta5-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Kvanta5 Core was listening for P2P connections, either using
...
- Kvanta5 Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/kvanta5/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Kvanta5 Core with the configuration
...
are now required to compile Kvanta5 Core. (#29091, #30263)

- The minimum required glibc to run Kvanta5 Core is now
...
- A new `-testdatadir` option has been added to `test_kvanta5` to allow specifying the
...
Previous releases of Kvanta5 Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
