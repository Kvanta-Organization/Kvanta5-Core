Kvanta5 Core version 0.17.0 is now available from:

  <https://bitcoincore.org/bin/kvanta5-core-0.17.0/>
...
Kvanta5 Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `kvanta5.conf` file, not inside included files or from
  ...
  as kvanta5.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the kvanta5.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=kvanta5
    test.uacomment=kvanta5-testnet
...
be used in V0.17 by starting kvanta5d with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Kvanta5 Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/kvanta5/bips/blob/master/bip-0174.mediawiki) is an interchange format for Kvanta5 transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://kvanta5talk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Kvanta5 transaction goes through the
following steps:
...
- An **Extractor** produces a valid Kvanta5 transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in KV5. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When kvanta5 is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If kvanta5 is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart kvanta5d with
...
- When running kvanta5d with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If kvanta5d
...
- #11962 `0a01843` add seed.kvanta5.sprovoost.nl to DNS seeds (Sjors)
...
- #11687 `98kv57f` External wallet files (ryanofsky)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in KV5 (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI kvanta5:// (krab)
...
- #13528 `b877c39` Move Kvanta5GUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libkvanta5-util to libkvanta5-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for kvanta5-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling kvanta5-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use Kvanta5TestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to kvanta5.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use Kvanta5TestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use Kvanta5TestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use Kvanta5TestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for Kvanta5TestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_kvanta5` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.kvanta5 is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_kvanta5` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in kvanta5d systemd service file (Flowdalic)
- #12652 `55f490a` kvanta5-cli: Provide a better error message when kvanta5d is not running (practicalswift)
...
- #12822 `ff48f62` Revert 7deba93bdc76616011a9f493ckv503d60084416f and fix expired-key-sigs properly (TheBlueMatt)
...
- #12769 `35eb9d6` Add systemd service to kvanta5d in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for kvanta5 address in translations (kaplanmaxe)
...
- #13603 `453ae5e` kvanta5-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix kvanta5-cli --version (Empact)
...
- #12877 `2b54155` Use kvanta5d in Tor documentation (knoxcard)
...
- #13346 `60f0358` update kvanta5-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed kvanta5-cli -help output for help2man (hebasto)
