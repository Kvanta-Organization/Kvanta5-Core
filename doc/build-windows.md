...
Below are some notes on how to build Kvanta5 Core for Windows.

The options known to work for building Kvanta5 Core on Windows are:
...
Note that for WSL the Kvanta5 Core source path MUST be somewhere in the default mount file system, for
example /usr/src/kvanta5, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\kvanta5`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/kvanta5
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/kvanta5 --strip
```
