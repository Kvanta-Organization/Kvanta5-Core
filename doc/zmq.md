...
The Kvanta5 Core daemon can be configured to act as a trusted "border
router", implementing the kvanta5 wire protocol and relay, making
...
The ZeroMQ feature in Kvanta5 Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ kvanta5d -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/kvanta5d.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in kvanta5.conf.
...
From the perspective of kvanta5d, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into kvanta5d directly. Furthermore, no information is
...
using. Kvanta5d appends an up-counting sequence number to each
