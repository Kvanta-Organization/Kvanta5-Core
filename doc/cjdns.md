# CJDNS support in Kvanta5 Core

It is possible to run Kvanta5 Core over CJDNS, an encrypted IPv6 network that
...
done outside of Kvanta5 Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Kvanta5 network and individual nodes.
...
before it will work with your Kvanta5 Core node. This is described in steps
...
## Run Kvanta5 Core with CJDNS

Once you are connected to the CJDNS network, the following Kvanta5 Core
...
When enabled, this option tells Kvanta5 Core that it is running in an
...
IPv6 local network. This helps Kvanta5 Core perform better address management:
...
CJDNS support was added to Kvanta5 Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `kvanta5-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/kvanta5/kvanta5/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Kvanta5 Core

There are several ways to see your CJDNS address in Kvanta5 Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `kvanta5-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `kvanta5-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `kvanta5-cli help getnodeaddresses` for details.
