...
- [Development guidelines](#development-guidelines)
    - [General Kvanta5 Core](#general-kvanta5-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Kvanta5 Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while kvanta5d is running with the `logging`
RPC.  For instance, launching kvanta5d with `-debug` or `-debug=1` will turn on
...
with "play kvanta5s" on a test network.
...
Kvanta5 Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at kvanta5d startup or
`kvanta5-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `kvanta5-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/kvanta5/kvanta5/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_kvanta5
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_kvanta5 --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/kvanta5d -printtoconsole
...
# A coverage report will now be accessible at `./build/test_kvanta5.coverage/index.html`,
...
To profile a running kvanta5d process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep kvanta5d` -- sleep 60
```
...
Kvanta5 Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`kvanta5d`)](https://doxygen.bitcoincore.org/kvanta5d_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `kvanta5d.cpp`. Responsible for starting up and
...
- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.bitcoincore.org/httpserver_8cpp.html#aa6a7kv57265043bc0193220c5ae3a55f)
...
However, in open source software such as Kvanta5 Core, where everyone uses
...
pay attention to for reviewers of Kvanta5 Core code.

General Kvanta5 Core
----------------------
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: Kvanta5 Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `KVANTA5_FOO_BAR_H`, e.g.

```c++
#ifndef KVANTA5_FOO_BAR_H
#define KVANTA5_FOO_BAR_H
...
#endif // KVANTA5_FOO_BAR_H
```
...
Some of these are maintained by active developers of Kvanta5 Core, in which case
...
a Kvanta5 Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/kvanta5-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/kvanta5-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/kvanta5-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/kvanta5-core/ctaes ; maintained by Core contributors.

- src/minisketch
  - Upstream at https://github.com/kvanta5-core/minisketch ; maintained by Core contributors.
...
file descriptors, it will cause problems with Kvanta5's `select()` loop, because
...
compatibility between nodes. This happened in Kvanta5 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Kvanta5's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/kvanta5/kvanta5/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/kvanta5/kvanta5/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/kvanta5/kvanta5/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `kvanta5-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `kvanta5-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Kvanta5 Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/kvanta5/kvanta5/pull/14635).
