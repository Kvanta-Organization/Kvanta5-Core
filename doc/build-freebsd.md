...
This guide describes how to build kvanta5d, command-line utilities, and GUI on FreeBSD.
...
### 2. Clone Kvanta5 Repo
Now that `git` and all the required dependencies are installed, let's clone the Kvanta5 Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `kvanta5d` or `kvanta5-qt`.
...
```
to: /path/to/kvanta5/depends/x86_64-unknown-freebsd[release-number]
```
...
Kvanta5 Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
