When Kvanta5 Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Kvanta5 node is listening,
or one where a service is listening that is not related to the Kvanta5 network.
As a result, this service may occasionally get connection attempts from Kvanta5
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Kvanta5 Core,
trying to connect because it thinks there is a Kvanta5 node on that
...
Below is a list of "bad" ports which Kvanta5 Core avoids when choosing a peer to
...
[pull/23306](https://github.com/kvanta5/kvanta5/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/kvanta5/kvanta5/pull/23542)
