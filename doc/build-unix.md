UNIX BUILD NOTES
====================
Some notes on how to build Kvanta5 Core in Unix.
...
memory available when compiling Kvanta5 Core. On systems with less, gcc can be
...
To build Kvanta5 Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/kvanta5/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/kvanta5/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Kvanta5 Core can
