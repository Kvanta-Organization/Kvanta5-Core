# PSBT Howto for Kvanta5 Core

Since Kvanta5 Core 0.17, an RPC interface exists for Partially Signed Kvanta5
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/kvanta5/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Kvanta5 transactions that are not fully signed
...
#### Multisig with multiple Kvanta5 Core instances
...
Kvanta5 Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `kvanta5-cli` is used.
...
Later, when *V* KV5 has been received on *Amulti*, and Bob and Carol want to
