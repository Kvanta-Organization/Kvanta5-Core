...
public:
    ...
    std::string operator()(const Kvanta5P2QRDestination& id) const
    {
        std::vector<unsigned char> data;
        data.reserve((id.size() * 8 + 4) / 5);
        ConvertBits<8, 5, true>(
            [&](unsigned char c) { data.push_back(c); },
            id.begin(),
            id.end()
        );
        return bech32::Encode(bech32::Encoding::BECH32M, m_params.Bech32QRHRP(), data);
    }
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    const bool is_bech32 =
        lower_str.substr(0, params.Bech32HRP().size()) == params.Bech32HRP() ||
        lower_str.substr(0, params.Bech32QRHRP().size()) == params.Bech32QRHRP();

    if (!is_bech32 && DecodeBase58Check(str, data, 21)) {
        // base58-encoded Kvanta5 addresses.
    ...
    if (dec.encoding == bech32::Encoding::BECH32M && dec.hrp == params.Bech32QRHRP()) {
        if (dec.data.empty()) {
            error_str = "Empty Bech32 QR data section";
            return CNoDestination();
        }

        data.clear();
        data.reserve((dec.data.size() * 5) / 8);

        if (!ConvertBits<5, 8, false>(
                [&](unsigned char c) { data.push_back(c); },
                dec.data.begin(),
                dec.data.end())) {
            error_str = "Invalid padding in Bech32 QR data section";
            return CNoDestination();
        }

        if (data.size() != uint256::size()) {
            error_str = strprintf("Invalid Bech32 QR address program size (%d bytes)", data.size());
            return CNoDestination();
        }
