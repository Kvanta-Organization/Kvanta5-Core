...
std::string GetTxnOutputType(TxoutType t)
{
    ...
    case TxoutType::KVANTA5_P2QR: return "kvanta5_p2qr";
...
TxoutType Solver(const CScript& scriptPubKey, std::vector<std::vector<unsigned char>>& vSolutionsRet)
{
    ...
    // Native Kvanta5 quantum-resistant output:
    //   OP_QuantumQuark <32-byte ML-DSA-87 pubkey commitment>
    //
    // This is deliberately not a witness program and not OP_RETURN.
    // OP_QuantumQuark is OP_RESERVED at the script byte level, so old
    // nodes fail closed if they try to execute it. Upgraded nodes recognize
    // TxoutType::KVANTA5_P2QR and validate it explicitly after activation.
    if (scriptPubKey.size() == 2 + KVANTA5_P2QR_PROGRAM_SIZE &&
        scriptPubKey[0] == OP_KVANTA5_P2QR &&
        scriptPubKey[1] == KVANTA5_P2QR_PROGRAM_SIZE) {
        vSolutionsRet.push_back(std::vector<unsigned char>(
            scriptPubKey.begin() + 2,
            scriptPubKey.end()
        ));
        return TxoutType::KVANTA5_P2QR;
    }
