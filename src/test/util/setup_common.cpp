...
constexpr inline auto TEST_DIR_PATH_ELEMENT{"test_common kvanta5"}; // Includes a space to catch possible path escape issues.
...
static CScript Kvanta5TestP2QRScript(unsigned char tag = 1)
{
...
{
    // TestChain100Setup is a private regtest fixture. It intentionally ignores
    // the caller's chain_type because it manufactures a local 100-block chain.
    //
    // Kvanta5 genesis is not Bitcoin's old test timestamp, so anchor mock time
    // to the active regtest genesis instead of inheriting Bitcoin's 2020-era
    // hardcoded timestamp.
    SetMockTime(std::chrono::seconds{Params().GenesisBlock().GetBlockTime() + Params().GetConsensus().nPowTargetSpacing});

    // Keep a deterministic legacy key only for older helper APIs that still
    // accept CKey parameters. The fixture chain itself mines to a Kvanta5 P2QR
    // script, not to OP_CHECKSIG.
...
{
    const CScript scriptPubKey = Kvanta5TestP2QRScript();
    ...
    for (int i = 0; i < num_blocks; ++i) {
        ...
        // Advance by Kvanta5's configured spacing. This keeps the fixture in
        // the same time domain as Kvanta5's DA instead of mining one-second
        // Bitcoin-style test blocks.
        SetMockTime(GetTime<std::chrono::seconds>() + std::chrono::seconds{Params().GetConsensus().nPowTargetSpacing});
    }
}
    ...
    Chainstate& chainstate)
{
    ...
    // Do not override block.nBits here. The BlockAssembler/consensus path must
    // select the expected Kvanta5 difficulty bits. Validation will reject blocks
    // whose header bits are merely "easy" but do not match the active DA rules.
    ...
    Chainstate* chainstate)
{
    ...
    const bool accepted = Assert(m_node.chainman)->ProcessNewBlock(shared_pblock, true, true, nullptr);
    assert(accepted);
                                                                                  ...
                                                                                  const std::optional<uint32_t>& fee_output)
{
    ...
    // This helper retains its inherited CKey/signature API for callers that
    // still exercise legacy script tests. Kvanta5-native P2QR transaction
    // helpers should be added separately rather than pretending ECDSA helpers
    // can sign ML-DSA/P2QR spends.
...
std::vector<CTransactionRef> TestChain100Setup::PopulateMempool(FastRandomContext& det_rand, size_t num_transactions, bool submit)
{
    ...
    while (num_transactions > 0 && !unspent_prevouts.empty()) {
        ...
        for (size_t n{0}; n < num_outputs; ++n) {
            CScript spk = Kvanta5TestP2QRScript(static_cast<unsigned char>((num_transactions + n) & 0xff));
...
void TestChain100Setup::MockMempoolMinFee(const CFeeRate& target_feerate)
{
    ...
    mtx.vout.emplace_back(1 * COIN, Kvanta5TestP2QRScript(42));
...
/**
 * Legacy Bitcoin fixture block retained only for inherited bloom/merkleblock
 * tests that still exercise generic serialization/filter behavior.
 *
 * Do not use this as a Kvanta5 consensus-valid block fixture.
 */
