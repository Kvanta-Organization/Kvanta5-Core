...
BOOST_FIXTURE_TEST_CASE(stale_tip_peer_management, OutboundTest)
{
    ...
    // Kvanta5's stale-tip condition is still 3 target blocks, which is
    // about 3 minutes at 60-second spacing. However,
    // CheckForStaleTipAndEvictPeers() is throttled by STALE_CHECK_INTERVAL
    // in net_processing.cpp, currently 10 minutes. Advance past that throttle
    // so this unit test actually exercises the stale-tip path.
