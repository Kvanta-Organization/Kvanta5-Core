...
static CScript Kvanta5TestP2QRScriptForTxTests(uint8_t tag = 0xC5)
{
...
{
    // Bitcoin's tx_valid.json is full of legacy P2PKH/P2SH/multisig/script
    // validity vectors. Kvanta5 intentionally rejects those as normal
    // transaction outputs/spends. Keep this test as a Kvanta5-native transaction
    // validity smoke test instead of importing Bitcoin's legacy-valid set.

    CMutableTransaction tx;
    tx.version = 1;
    tx.vin.resize(1);
    tx.vin[0].prevout = COutPoint{Txid::FromUint256(uint256::ONE), 0};
    tx.vin[0].scriptSig = CScript() << std::vector<unsigned char>(64, 0x01) << std::vector<unsigned char>(2592, 0x02);
    tx.vin[0].nSequence = CTxIn::SEQUENCE_FINAL;

    tx.vout.resize(1);
    tx.vout[0].nValue = COIN;
    tx.vout[0].scriptPubKey = Kvanta5TestP2QRScriptForTxTests();

    CTransaction tx_const{tx};

    BOOST_CHECK(!tx_const.IsCoinBase());
    BOOST_CHECK(!tx_const.IsNull());
    BOOST_CHECK(MoneyRange(tx_const.vout[0].nValue));
    BOOST_CHECK(tx_const.vout[0].scriptPubKey == Kvanta5TestP2QRScriptForTxTests());
}

BOOST_AUTO_TEST_CASE(tx_invalid)
{
    // Bitcoin's tx_invalid.json is a legacy Bitcoin script fixture set.
    // Kvanta5 deliberately changed the valid transaction/output universe, so
    // those inherited vectors are no longer a meaningful invalidity oracle.
    //
    // Keep this test focused on transaction-level invalid structures that must
    // remain invalid for Kvanta5.

    {
        ...
        tx.vout.resize(1);
        tx.vout[0].nValue = COIN;
        tx.vout[0].scriptPubKey = Kvanta5TestP2QRScriptForTxTests(0xD0);
...

    {
        ...
        tx.vout.resize(1);
        tx.vout[0].nValue = -1;
        tx.vout[0].scriptPubKey = Kvanta5TestP2QRScriptForTxTests(0xD1);
    ...
    {
        // Output value above money range.
        CMutableTransaction tx;
        tx.version = 1;
        tx.vin.resize(1);
        tx.vin[0].prevout = COutPoint{Txid::FromUint256(uint256::ONE), 0};
        ...
        tx.vout[0].scriptPubKey = Kvanta5TestP2QRScriptForTxTests(0xD2);
...

    {
        ...
        tx.vin.resize(2);
        tx.vin[0].prevout = COutPoint{Txid::FromUint256(uint256::ONE), 0};
        tx.vin[1].prevout = COutPoint{Txid::FromUint256(uint256::ONE), 0};
        tx.vin[0].scriptSig = CScript() << std::vector<unsigned char>(64, 0x01) << std::vector<unsigned char>(2592, 0x02);
        tx.vin[1].scriptSig = CScript() << std::vector<unsigned char>(64, 0x03) << std::vector<unsigned char>(2592, 0x04);
        tx.vin[0].nSequence = CTxIn::SEQUENCE_FINAL;
        tx.vin[1].nSequence = CTxIn::SEQUENCE_FINAL;
        tx.vout.resize(1);
        tx.vout[0].nValue = COIN;
        tx.vout[0].scriptPubKey = Kvanta5TestP2QRScriptForTxTests(0xD3);
...
{
    // Bitcoin's original basic transaction test deserializes a hardcoded
    // legacy transaction and expects it to be valid. Kvanta5's valid output
    // universe is P2QR / wrapped P2QR, so use a native structural-validity
    // transaction here.
    ...
    tx.vin[0].scriptSig = CScript()
        ...
        << std::vector<unsigned char>(2592, 0x02);
    tx.vin[0].nSequence = CTxIn::SEQUENCE_FINAL;

    tx.vout.resize(1);
    tx.vout[0].nValue = COIN;
    tx.vout[0].scriptPubKey = Kvanta5TestP2QRScriptForTxTests(0xE0);

    TxValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransaction(CTransaction{tx}, state), "Kvanta5-native transaction should be structurally valid");
}
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    CheckIsNotStandard(t, "p2qr-tx-size-deferred");
