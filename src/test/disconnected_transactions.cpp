...
BOOST_AUTO_TEST_CASE(disconnectpool_memory_limits)
{
    ...
    // Kvanta5 coinbase transactions are not all uniform anymore. The early
    // setup chain can include larger/different coinbase shapes, e.g. dev-fund
    // related outputs. This test is about DisconnectedBlockTransactions memory
    // limiting, not coinbase layout, so use a uniform-size subset.
    const size_t expected_tx_usage{RecursiveDynamicUsage(block_vtx.back())};
    block_vtx.erase(std::remove_if(block_vtx.begin(), block_vtx.end(),
                                   [&](const CTransactionRef& tx) {
                                       return RecursiveDynamicUsage(tx) != expected_tx_usage;
                                   }),
                    block_vtx.end());

    BOOST_REQUIRE_GE(block_vtx.size(), 2);
    ...
    size_t usage_full{0};
    {
        const size_t USAGE_ALL_OVERESTIMATE{MAP_100 + ENTRY_USAGE_ESTIMATE * block_vtx.size()};
        DisconnectedBlockTransactions disconnectpool{USAGE_ALL_OVERESTIMATE};
        auto evicted_txns{disconnectpool.AddTransactionsFromBlock(block_vtx)};
        BOOST_CHECK_EQUAL(evicted_txns.size(), 0);
        BOOST_CHECK(disconnectpool.DynamicMemoryUsage() <= USAGE_ALL_OVERESTIMATE);
