...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    s << "02"                                    // network type (IPv6)
         "10"                                    // address length
         "fd6b88c08724ca978112ca1bbdcafac2"_hex; // address: 0xfd + sha256("kvanta5")[0:5] +
                                                 // sha256(name)[0:10]
    s >> ser_params(addr);
...
BOOST_AUTO_TEST_CASE(initial_advertise_from_version_message)
{
    ...
    // Upstream Bitcoin regtest starts this test in IBD because its regtest
    // genesis timestamp is ancient. Kvanta5 regtest uses a fresh genesis
    // timestamp, so the tip can already be recent enough to latch IBD=false.
    // Make the tip stale relative to mock time, reset the latch, then use the
    // strict helper as intended.
    SetMockTime(chainman.ActiveChain().Tip()->GetBlockTime() + 3 * 24 * 60 * 60);
    chainman.ResetIbd();
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Bitcoin's original 4,005,000-byte payload exceeded its transport
        // limit. Kvanta5 intentionally raises transport limits for 32 MB
        // blocks, so use a payload above the Kvanta5 transport ceiling.
        auto msg_data_3 = m_rng.randbytes<uint8_t>(41000000);
