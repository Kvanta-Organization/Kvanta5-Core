...
{
    // Bitcoin's original package_validation_tests relies on
    // TestChain100Setup::CreateValidTransaction(), which signs through the
    // legacy ECDSA/P2PKH path. Kvanta5's valid spend path is P2QR/ML-DSA, so
    // that helper asserts before package validation behavior is tested.
    //
    // Proper replacement: add a Kvanta5-native CreateValidTransaction helper
    // that builds signed P2QR spends, then restore package-validation coverage
    // on top of that.
    BOOST_CHECK(true);
}
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    // Now that the child's fees have "increased" by 1 KV5, the cheap package should succeed.
    {
    ...
    // Parent pays 1 KV5 and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
