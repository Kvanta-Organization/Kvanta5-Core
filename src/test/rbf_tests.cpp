...
BOOST_FIXTURE_TEST_CASE(improves_feerate, TestChain100Setup)
{
    ...
    // The removed conflict set is the 3-transaction chain tx1 -> tx2 -> tx5.
    // This topology is uncalculable from either endpoint:
    //   tx1 has two descendants: tx2 and tx5
    //   tx5 has two ancestors: tx1 and tx2
    //
    // Bitcoin's original test asserted the tx5/ancestor endpoint. Kvanta5's
    // transaction serialization changes txids, so deterministic set ordering
    // may visit tx1 first and report the descendant endpoint instead. Both
    // messages prove the same package-topology violation.
    BOOST_CHECK_MESSAGE(
        res3.value().second == strprintf("%s has 2 ancestors, max 1 allowed", tx5->GetHash().GetHex()) ||
        res3.value().second == strprintf("%s has 2 descendants, max 1 allowed", tx1->GetHash().GetHex()),
        "unexpected RBF topology failure message: " + res3.value().second);
}
