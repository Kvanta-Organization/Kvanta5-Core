...
{
    TestArgsManager test_args;
    const auto testnet5 = std::make_pair("-testnet5", ArgsManager::ALLOW_ANY);
    const auto regtest = std::make_pair("-regtest", ArgsManager::ALLOW_ANY);
    test_args.SetupArgs({testnet5, regtest});

    const char* argv_testnet5[] = {"cmd", "-testnet5"};
    const char* argv_regtest[] = {"cmd", "-regtest"};
    const char* argv_test5_no_reg[] = {"cmd", "-testnet5", "-noregtest"};
    const char* argv_both[] = {"cmd", "-testnet5", "-regtest"};

    // regtest in testnet5 section is ignored unless the selected config
    // network is testnet5.
    const char* testnet5conf = "testnet5=1\nregtest=0\n[testnet5]\nregtest=1";
    std::string error;

    BOOST_CHECK(test_args.ParseParameters(0, argv_testnet5, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "main");

    BOOST_CHECK(test_args.ParseParameters(2, argv_testnet5, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "testnet5");

    BOOST_CHECK(test_args.ParseParameters(2, argv_regtest, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "regtest");

    BOOST_CHECK(test_args.ParseParameters(3, argv_test5_no_reg, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "testnet5");
    ...
    BOOST_CHECK(test_args.ParseParameters(0, argv_testnet5, error));
    test_args.ReadConfigString(testnet5conf);
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "testnet5");
    ...
    // Check setting the config network to testnet5 doesn't break things.
    test_args.SelectConfigNetwork("testnet5");
            ...
            ForEachNoDup(conf_actions, SET, SECTION_NEGATE, [&] {
                for (bool soft_set : {false, true}) {
                    for (bool force_set : {false, true}) {
                        for (const std::string& section : {ChainTypeToString(ChainType::MAIN), ChainTypeToString(ChainType::TESTNET5), ChainTypeToString(ChainType::SIGNET)}) {
                            for (const std::string& network : {ChainTypeToString(ChainType::MAIN), ChainTypeToString(ChainType::TESTNET5), ChainTypeToString(ChainType::SIGNET)}) {
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_kvanta5 --run_test=argsman_tests/util_ArgsMerge
    //
    ...
    BOOST_CHECK_EQUAL(out_sha_hex, "8207d4b1c3d42e85867c6eca78c6419db73fe0c5b9150fdd2d41f2b1f63d90d9");
}
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions) {
        ...
        parser.AddArg("-testnet5", "testnet5", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);

        auto arg = [](Action action) { return action == ENABLE_TEST  ? "-testnet5=1"   :
                                              action == DISABLE_TEST ? "-testnet5=0"   :
                                              action == NEGATE_TEST  ? "-notestnet5=1" :
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_kvanta5 --run_test=argsman_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
    //
    //   <input> || <output>
    BOOST_CHECK_EQUAL(out_sha_hex, "8eef853cc3d498cb260f09e13aecd94b6a9817b6f3cd192e4209a73d95965e74");
