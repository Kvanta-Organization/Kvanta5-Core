...
static CScript Kvanta5TestP2QRScript(uint8_t tag)
{
...
static const char* ReadStatusName(ReadStatus status)
{
    switch (status) {
    case READ_STATUS_OK:
    ...
    return "READ_STATUS_UNKNOWN";
}

BOOST_FIXTURE_TEST_SUITE(blockencodings_tests, RegTestingSetup)

static CMutableTransaction BuildTransactionTestCase(uint8_t output_tag = 0x01)
{
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = 42;
    tx.vout[0].scriptPubKey = Kvanta5TestP2QRScript(output_tag);
    return tx;
}
...
{
    CBlock block;
    CMutableTransaction tx = BuildTransactionTestCase(0x01);
    ...
    tx.vout[0].scriptPubKey = Kvanta5TestP2QRScript(0x21);
    ...
    tx.vout[0].scriptPubKey = Kvanta5TestP2QRScript(0x41);
...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest)
{
    ...
    // Do a simple ShortTxIDs RT
    {
        ...
        CBlock block3;
        {
            const ReadStatus status = partialBlock.FillBlock(block3, {block.vtx[1]});
            BOOST_TEST_MESSAGE("SimpleRoundTripTest FillBlock status: " << ReadStatusName(status));
            BOOST_CHECK(status == READ_STATUS_OK);
        }
...
BOOST_AUTO_TEST_CASE(NonCoinbasePreforwardRTTest)
{
    ...
    // Test with pre-forwarding tx 1, but not coinbase
    {
        ...
        {
            const ReadStatus status = partialBlock.FillBlock(block3, {block.vtx[0]});
            BOOST_TEST_MESSAGE("NonCoinbasePreforwardRTTest FillBlock status: " << ReadStatusName(status));
            BOOST_CHECK(status == READ_STATUS_OK);
        }
        BOOST_CHECK_EQUAL(block.GetHash().ToString(), block3.GetHash().ToString());
        BOOST_CHECK_EQUAL(block.hashMerkleRoot.ToString(), BlockMerkleRoot(block3, &mutated).ToString());
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest)
{
    ...
    // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
    {
        ...
        PartiallyDownloadedBlock partialBlockCopy = partialBlock;
        {
            const ReadStatus status = partialBlock.FillBlock(block2, {});
            BOOST_TEST_MESSAGE("SufficientPreforwardRTTest FillBlock status: " << ReadStatusName(status));
            BOOST_CHECK(status == READ_STATUS_OK);
        }
...
{
    CTxMemPool& pool = *Assert(m_node.mempool);
    CMutableTransaction coinbase = BuildTransactionTestCase(0x61);
    ...
    // Test simple header round-trip with only coinbase
    {
        ...
        {
            const ReadStatus status = partialBlock.FillBlock(block2, vtx_missing);
            BOOST_TEST_MESSAGE("EmptyBlockRoundTripTest FillBlock status: " << ReadStatusName(status));
            BOOST_CHECK(status == READ_STATUS_OK);
        }
        BOOST_CHECK_EQUAL(block.GetHash().ToString(), block2.GetHash().ToString());
        BOOST_CHECK_EQUAL(block.hashMerkleRoot.ToString(), BlockMerkleRoot(block2, &mutated).ToString());
...
BOOST_AUTO_TEST_CASE(ReceiveWithExtraTransactions)
{
    ...
    CMutableTransaction mtx = BuildTransactionTestCase(0x81);
