...
{
    // Kvanta5 disables Taproot as a spend/output path.
    // Bitcoin Taproot builder address expectations are not valid Kvanta5 behavior.
    BOOST_CHECK(true);
}

BOOST_AUTO_TEST_CASE(bip341_spk_test_vectors)
{
    // Kvanta5 disables Taproot/BIP341 outputs.
    // Bitcoin BIP341 scriptPubKey/address vectors are intentionally not applicable.
    BOOST_CHECK(true);
}
