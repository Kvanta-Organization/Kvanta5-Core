...
static constexpr auto INBOUND_INVENTORY_BROADCAST_INTERVAL{1s};
...
static bool Kvanta5RelayReadPushesOnly(
    ...
    std::vector<std::vector<unsigned char>>& pushes_out)
{
    ...
    while (pc < script_sig.end()) {
        ...
        if (!script_sig.GetOp(pc, opcode, data)) {
            return false;
        }
    ...
    return !pushes_out.empty();
}

static bool Kvanta5RelayIsP2QRRedeemScript(const std::vector<unsigned char>& raw_script)
{
    const CScript redeem_script(raw_script.begin(), raw_script.end());

    std::vector<std::vector<unsigned char>> solutions;
    const TxoutType type = Solver(redeem_script, solutions);

    return type == TxoutType::KVANTA5_P2QR &&
           solutions.size() == 1 &&
           solutions[0].size() == KVANTA5_P2QR_PROGRAM_SIZE;
}

static bool Kvanta5RelayIsSingleSigP2QRSpendPushes(
    const std::vector<std::vector<unsigned char>>& pushes)
{
    if (pushes.size() != 2) {
        return false;
    }
    ...
    return consensus::kvanta5p2qr::ParseSpendData(native_spend_script, sig, pubkey, error);
}

static bool Kvanta5RelayIsMultisigP2QRSpendPushes(
    const std::vector<std::vector<unsigned char>>& pushes)
{
    Kvanta5P2QRMultisigSpend spend;
    std::string error;

    return Kvanta5ParseP2QRMultisigSpend(pushes, spend, &error);
}

static bool IsKvanta5P2QRRelaySpendEnvelope(const CScript& script_sig)
{
    std::vector<std::vector<unsigned char>> pushes;
    if (!Kvanta5RelayReadPushesOnly(script_sig, pushes)) {
        return false;
    }

    /*
     * Native single-sig P2QR:
     *
     *   <sig> <pubkey>
     */
    if (Kvanta5RelayIsSingleSigP2QRSpendPushes(pushes)) {
        return true;
    }

    /*
     * Native multisig P2QR:
     *
     *   <sig_0> ... <sig_m-1>
     *   <idx_0> ... <idx_m-1>
     *   <pubkey_0> ... <pubkey_n-1>
     *   <m> <n>
     */
    if (Kvanta5RelayIsMultisigP2QRSpendPushes(pushes)) {
        return true;
    }

    /*
     * Wrapped P2SH-carried single-sig P2QR:
     *
     *   <sig> <pubkey> <redeemScript>
     *
     * redeemScript must be:
     *
     *   50 20 <program>
     */
    if (pushes.size() == 3 && Kvanta5RelayIsP2QRRedeemScript(pushes.back())) {
        std::vector<std::vector<unsigned char>> inner_pushes{pushes.begin(), pushes.end() - 1};
        if (Kvanta5RelayIsSingleSigP2QRSpendPushes(inner_pushes)) {
            return true;
        }
    }

    /*
     * Wrapped P2SH-carried multisig P2QR:
     *
     *   <sig_0> ... <sig_m-1>
     *   <idx_0> ... <idx_m-1>
     *   <pubkey_0> ... <pubkey_n-1>
    ...
    if (pushes.size() > 3 && Kvanta5RelayIsP2QRRedeemScript(pushes.back())) {
        std::vector<std::vector<unsigned char>> inner_pushes{pushes.begin(), pushes.end() - 1};
        if (Kvanta5RelayIsMultisigP2QRSpendPushes(inner_pushes)) {
            return true;
        }
    }
...
static int32_t GetKvanta5P2QRRelayFeeVSize(const TxMempoolInfo& txinfo)
{
    static constexpr int32_t P2QR_RELAY_FEE_VBYTES_PER_INPUT{68};

    int64_t p2qr_scriptsig_bytes{0};
    int64_t p2qr_inputs{0};

    for (const CTxIn& txin : txinfo.tx->vin) {
        if (txin.scriptSig.empty()) {
            continue;
        }

        if (!IsKvanta5P2QRRelaySpendEnvelope(txin.scriptSig)) {
            continue;
        }
    ...
    const int64_t discounted_p2qr_part = p2qr_inputs * P2QR_RELAY_FEE_VBYTES_PER_INPUT;

    /*
     * txinfo.vsize is raw mempool vsize. For relay-feefilter purposes only,
     * price large P2QR scriptSig envelopes as compact modern signature spends.
     */
    int64_t adjusted_vsize =
        static_cast<int64_t>(txinfo.vsize) -
        p2qr_scriptsig_bytes +
        discounted_p2qr_part;

    if (adjusted_vsize < 1) {
        adjusted_vsize = 1;
    }

    if (adjusted_vsize > std::numeric_limits<int32_t>::max()) {
        adjusted_vsize = std::numeric_limits<int32_t>::max();
    }

    return static_cast<int32_t>(adjusted_vsize);
}
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                if (fSendTrickle && tx_relay->m_send_mempool) {
                    ...
                    for (const auto& txinfo : vtxinfo) {
                        ...
                        if (txinfo.fee < filterrate.GetFee(GetKvanta5P2QRRelayFeeVSize(txinfo))) {
                            continue;
                        }
                        if (tx_relay->m_bloom_filter) {
