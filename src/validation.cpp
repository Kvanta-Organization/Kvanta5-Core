...
static constexpr unsigned int MAX_P2QR_RAW_BLOCK_WEIGHT = MAX_BLOCK_WEIGHT;
...
static bool AllOutputsNativeP2QR(const CTransaction& tx)
{
    if (tx.vout.empty()) {
        return false;
    }
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (!consensus::kvanta5p2qr::IsOutput(txout.scriptPubKey, ignored_program)) {
            return false;
        }
    }
...
static bool HasKvanta5P2QRInput(const CTransaction& tx, const CCoinsViewCache& view)
{
    for (const CTxIn& txin : tx.vin) {
        ...
        if (consensus::kvanta5p2qr::IsOutput(coin.out.scriptPubKey, ignored_program) ||
            consensus::kvanta5p2qr::IsP2SHOutput(coin.out.scriptPubKey, ignored_script_id)) {
            return true;
        }
    }
...
static bool OversizedTxAllowedByP2QRPolicy(const CTransaction& tx, const CCoinsViewCache& view)
{
    /*
     * Oversized institutional/consolidation transaction policy:
     *
     *   - every output must be native P2QR
     *   - at least one input must spend native P2QR or P2SH-wrapped P2QR
     *
     * This allows old wrapped mining payouts to be consolidated/migrated into
     * native P2QR outputs, while still preventing ordinary oversized legacy
     * transactions from using the P2QR exception.
     */
    if (!AllOutputsNativeP2QR(tx)) {
        return false;
    }

    if (!HasKvanta5P2QRInput(tx, view)) {
        return false;
    }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Rather not work on nonstandard transactions. Native KVANTA5_P2QR spends are
    // allowed to exceed legacy scriptSig size policy and, in restricted cases,
    // normal tx-size policy. Both exceptions need prevout context, so defer them
    // until inputs are loaded into m_view below.
    std::string reason;
    bool fDeferredP2QRScriptSigSize = false;
    bool fDeferredP2QRTxSize = false;
    ...
    if (m_pool.m_opts.require_standard &&
        ...
        if (reason == "p2qr-scriptsig-size-deferred") {
            fDeferredP2QRScriptSigSize = true;

             LogPrintf("KVANTA5_P2QR policy debug: deferring P2QR scriptSig-size standardness check until inputs are available: tx=%s\n",
                tx.GetHash().ToString());
        } else if (reason == "p2qr-tx-size-deferred") {
            fDeferredP2QRTxSize = true;

            LogPrintf("KVANTA5_P2QR policy debug: deferring tx-size standardness check until inputs are available: tx=%s weight=%u\n",
                tx.GetHash().ToString(), GetTransactionWeight(tx));
         } else {
    ...
    // Kvanta5 hardening:
    //
    // A same-prevout mempool conflict must fail closed unless the existing
    // mempool transaction explicitly opts in to BIP125 replacement, or inherits
    // BIP125 replaceability from an unconfirmed ancestor.
    //
    // Without this check, the normal mempool path behaves like full-RBF:
    // a higher-fee TX_B can evict a non-RBF TX_A spending the same prevout.
    // That is unacceptable for Kvanta5 wallet/exchange/pool safety.
    for (const CTxIn& txin : tx.vin) {
        ...
        if (ptxConflicting) {
            ...
            const RBFTransactionState rbf_state = IsRBFOptIn(*ptxConflicting, m_pool);
            if (rbf_state != RBFTransactionState::REPLACEABLE_BIP125) {
                return state.Invalid(
                    TxValidationResult::TX_MEMPOOL_POLICY,
                    "txn-mempool-conflict",
                    strprintf(
                        "conflicting transaction %s does not signal BIP125 replaceability",
    ...
    // Native P2QR oversized tx-size policy:
    //
    // IsStandardTx() cannot inspect prevouts, so it only deferred the tx-size
    // decision with reason "p2qr-tx-size-deferred". Now all inputs are known in
    // m_view, so enforce the actual P2QR-aware policy:
    //
    //   - tx weight must not exceed MAX_STANDARD_P2QR_TX_WEIGHT
    //   - all outputs must be native P2QR
    //   - at least one input must spend native P2QR
    //
    // This allows:
    //   - all P2QR inputs -> all P2QR outputs
    //   - mixed inputs including P2QR -> all P2QR outputs
    //
    // This rejects:
    //   - oversized txs with any non-P2QR output
    //   - oversized txs with only legacy/segwit/taproot inputs
    if (fDeferredP2QRTxSize) {
        if (GetTransactionWeight(tx) > MAX_STANDARD_P2QR_TX_WEIGHT) {
            return state.Invalid(TxValidationResult::TX_NOT_STANDARD, "tx-size");
        }

        if (!OversizedTxAllowedByP2QRPolicy(tx, m_view)) {
            return state.Invalid(TxValidationResult::TX_NOT_STANDARD, "tx-size");
        }

        LogPrintf("KVANTA5_P2QR policy debug: accepted oversized P2QR tx-size exception: tx=%s weight=%u\n",
            tx.GetHash().ToString(), GetTransactionWeight(tx));
    }

    // Native and P2SH-carried KVANTA5_P2QR spends carry large ML-DSA-87 spend
    // envelopes in scriptSig. Enforce legacy scriptSig size policy for all
    // non-P2QR inputs, and validate the P2QR envelope shape for oversized P2QR
    // inputs before allowing the transaction to continue to consensus script checks.
    if (m_pool.m_opts.require_standard && (fDeferredP2QRScriptSigSize || fDeferredP2QRTxSize)) {
        for (const CTxIn& txin : tx.vin) {
            ...
            const bool is_native_p2qr =
                consensus::kvanta5p2qr::IsOutput(coin.out.scriptPubKey, program);

            const bool is_p2sh_carried_p2qr =
                !is_native_p2qr &&
                consensus::kvanta5p2qr::IsP2SHOutput(coin.out.scriptPubKey, script_id);

            if (!is_native_p2qr && !is_p2sh_carried_p2qr) {
                return state.Invalid(TxValidationResult::TX_NOT_STANDARD, "scriptsig-size");
            }
            ...
            while (pc < txin.scriptSig.end()) {
                ...
                if (!txin.scriptSig.GetOp(pc, opcode, data)) {
                    return state.Invalid(
                        ...
                        is_native_p2qr ? "kv5-p2qr-scriptsig-format" : "kv5-p2qr-p2sh-scriptsig-format",
                        "failed to parse KVANTA5_P2QR scriptSig pushes");
                }
            ...
            if (is_native_p2qr) {
                ...
                //   <m> <n>
                if (pushes.size() == 2) {
                    std::vector<unsigned char> sig = pushes[0];
                    std::vector<unsigned char> pubkey = pushes[1];

                    CScript native_spend_script;
                    native_spend_script << sig << pubkey;

                    if (!consensus::kvanta5p2qr::ParseSpendData(native_spend_script, sig, pubkey, p2qr_error)) {
                        return state.Invalid(
                            TxValidationResult::TX_NOT_STANDARD,
                            "kv5-p2qr-scriptsig-format",
                            p2qr_error);
                    }
                } else {
                    Kvanta5P2QRMultisigSpend multisig_spend;
                    if (!Kvanta5ParseP2QRMultisigSpend(pushes, multisig_spend, &p2qr_error)) {
                        return state.Invalid(
                            TxValidationResult::TX_NOT_STANDARD,
                            "kv5-p2qr-scriptsig-format",
                            p2qr_error);
                ...
                LogPrintf("KVANTA5_P2QR policy debug: accepted native P2QR oversized scriptSig envelope for prevout=%s:%u\n",
                    txin.prevout.hash.ToString(), txin.prevout.n);
            } else {
                // P2SH-carried single-key:
                //   <sig> <pubkey> <redeemScript>
                //
                // P2SH-carried multisig:
                //   <sig_0> ... <sig_m-1>
                //   <idx_0> ... <idx_m-1>
                //   <pubkey_0> ... <pubkey_n-1>
                //   <m> <n>
                ...
                if (pushes.size() < 3) {
                    return state.Invalid(
                        TxValidationResult::TX_NOT_STANDARD,
                        "kv5-p2qr-p2sh-scriptsig-format",
                        strprintf("expected at least 3 pushes for P2SH-carried KVANTA5_P2QR spend, got %u", pushes.size()));
                }
                ...
                uint256 redeem_program;
                if (!consensus::kvanta5p2qr::IsOutput(redeem_script, redeem_program)) {
                    return state.Invalid(
                        TxValidationResult::TX_NOT_STANDARD,
                        "kv5-p2qr-p2sh-redeemscript",
                        "P2SH-carried KVANTA5_P2QR redeemScript is not native KVANTA5_P2QR");
                }

                if (CScriptID(redeem_script) != script_id) {
                    return state.Invalid(
                        TxValidationResult::TX_NOT_STANDARD,
                        "kv5-p2qr-p2sh-redeemscript-hash",
                        "P2SH-carried KVANTA5_P2QR redeemScript hash does not match prevout");
                }
                ...
                LogPrintf("KVANTA5_P2QR policy debug: accepted P2SH-carried P2QR oversized scriptSig envelope for prevout=%s:%u\n",
                    txin.prevout.hash.ToString(), txin.prevout.n);
            }
        }
    }
    ...
    const int64_t p2qr_adjusted_tx_weight = GetP2QRAdjustedTransactionWeight(*ws.m_ptx, m_view);

    ws.m_tx_handle = m_subpackage.m_changeset->StageAddition(ptx, ws.m_base_fees, nAcceptTime, m_active_chainstate.m_chain.Height(), entry_sequence, fSpendsCoinbase, nSigOpsCost, lock_points.value(), p2qr_adjusted_tx_weight);
    ...
    // KVANTA5_P2QR fee policy:
    //
    // ws.m_vsize remains the real transaction virtual size for accounting,
    // package limits, block limits, RPC reporting, and standardness.
    //
    // nFeeVirtualSize is only used for fee-rate threshold checks, so native
    // and P2SH-carried KVANTA5_P2QR signature envelopes are not priced like normal
    // transaction bytes.
    const int64_t nFeeVirtualSize = GetP2QRFeeVirtualSize(*ws.m_ptx, m_view, ws.m_vsize);

    if (!bypass_limits && ws.m_ptx->version != TRUC_VERSION && ws.m_modified_fees < m_pool.m_opts.min_relay_feerate.GetFee(nFeeVirtualSize)) {
        // Even though this is a fee-related failure, this result is TX_MEMPOOL_POLICY, not
        // TX_RECONSIDERABLE, because it cannot be bypassed using package validation.
        return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "min relay fee not met",
                             strprintf("%d < %d", ws.m_modified_fees, m_pool.m_opts.min_relay_feerate.GetFee(nFeeVirtualSize)));
    }

    // No individual transactions are allowed below the mempool min feerate except from disconnected
    // blocks and transactions in a package. Package transactions will be checked using package
    // feerate later.
    if (!bypass_limits && !args.m_package_feerates && !CheckFeeRate(nFeeVirtualSize, ws.m_modified_fees, state)) {
        return false;
    }
    ...
    if (ws.m_conflicts.size() == 1 && args.m_allow_carveouts) {
        ...
        // check is accomplished later, so we only ensure a limited set of transactions are RBF'able despite
        // mempool conflicts here. If our policy changes, we may need to move that check to here instead of
        // removing it wholesale.
        //
        // Such transactions are clearly not merging any existing packages, so we only care about ensuring
        // that package limits remain sane after removing the direct conflict and its descendants.
    ...
    if (auto ancestors{m_subpackage.m_changeset->CalculateMemPoolAncestors(ws.m_tx_handle, maybe_rbf_limits)}) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/kvanta5-dev/2018-November/016518.html
    ...
    // that we have the set of all ancestors we can detect this pathological case by
    // making sure ws.m_conflicts and ws.m_ancestors don't intersect.
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (tip && tip->nHeight + 1 >= m_active_chainstate.m_chainman.GetConsensus().nP2QRActivationHeight) {
        scriptVerifyFlags |= SCRIPT_VERIFY_KVANTA5_P2QR;
    }
...
{
    /*
     * Kvanta5 monetary policy:
     *
     * Height 0:
     *   Genesis block.
     *
     * Height 1:
     *   One-time consensus dev fund allocation:
     *   21,000,000 KV5.
     *
     * Height 2 onward:
     *   Normal mining subsidy begins at 50 KV5.
     *
     * This preserves:
     *   210,000,000 KV5 mineable emission
     *   + 21,000,000 KV5 consensus dev fund
     *   = 231,000,000 KV5 max supply.
     */
    if (nHeight == KVANTA5_DEVFUND_BLOCK_HEIGHT) {
        return KVANTA5_DEVFUND_TOTAL_AMOUNT;
    }

    if (nHeight < KVANTA5_FIRST_NORMAL_SUBSIDY_HEIGHT) {
        return 0;
    }

    const int adjusted_height = nHeight - KVANTA5_FIRST_NORMAL_SUBSIDY_HEIGHT;
    ...
    // Kvanta5 normal mining subsidy halves every 2,100,000 one-minute blocks.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    ScriptError error{SCRIPT_ERR_UNKNOWN_ERROR};
        ...
        uint256 kvanta5_p2qr_program;
    if (consensus::kvanta5p2qr::IsOutput(m_tx_out.scriptPubKey, kvanta5_p2qr_program)) {
        std::string p2qr_error;

        if (!(nFlags & SCRIPT_VERIFY_KVANTA5_P2QR)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, native KVANTA5_P2QR inactive",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
                ptxTo->vin[nIn].prevout.n);
            return std::make_pair(SCRIPT_ERR_BAD_OPCODE, std::move(debug_str));
        }

        if (!txdata || !txdata->m_spent_outputs_ready) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, missing spent outputs for native KVANTA5_P2QR validation",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
                ptxTo->vin[nIn].prevout.n);
            return std::make_pair(SCRIPT_ERR_UNKNOWN_ERROR, std::move(debug_str));
        }

        if (!consensus::kvanta5p2qr::CheckSpend(*ptxTo, nIn, m_tx_out, txdata->m_spent_outputs, p2qr_error)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, native KVANTA5_P2QR validation failed: %s",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
    ...
    if (consensus::kvanta5p2qr::IsP2SHWrappedSpendCandidate(m_tx_out.scriptPubKey, scriptSig)) {
        std::string p2qr_error;

        if (!(nFlags & SCRIPT_VERIFY_KVANTA5_P2QR)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, P2SH-carried KVANTA5_P2QR inactive",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
        ...
        if (!txdata || !txdata->m_spent_outputs_ready) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, missing spent outputs for P2SH-carried KVANTA5_P2QR validation",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
        ...
        if (!consensus::kvanta5p2qr::CheckSpendP2SHWrapped(*ptxTo, nIn, m_tx_out, txdata->m_spent_outputs, p2qr_error)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, P2SH-carried KVANTA5_P2QR validation failed: %s",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/kvanta5/kvanta5/issues/22596 for additional information.
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    if (block_index.nHeight >= consensusparams.nP2QRActivationHeight) {
    flags |= SCRIPT_VERIFY_KVANTA5_P2QR;
    }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    const bool p2qr_active =
    pindex->nHeight >= params.GetConsensus().nP2QRActivationHeight;

    // Coinbase has no P2QR spend auth, so raw weight is fine.
    int64_t p2qr_adjusted_block_weight = block.vtx.empty() ? 0 : GetTransactionWeight(*block.vtx[0]);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
         ...
         // Native P2QR adjusted block weight must be computed while prevouts
         // are still available in view. UpdateCoins() below spends/removes them.
         if (p2qr_active) {
             p2qr_adjusted_block_weight += GetP2QRAdjustedTransactionWeight(tx, view);
         }
       }
    ...
    if (state.IsValid()) {
        if (p2qr_active) {
            if (p2qr_adjusted_block_weight > MAX_BLOCK_WEIGHT) {
               state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-weight",
                             strprintf("P2QR-adjusted block weight too high: %d > %d",
                                      p2qr_adjusted_block_weight, MAX_BLOCK_WEIGHT));
           }
       } else {
           if (GetBlockWeight(block) > MAX_BLOCK_WEIGHT) {
               state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-weight",
                             strprintf("block weight too high: %d > %d",
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Size limits
    if (block.vtx.empty()) {
    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-length", "empty block");
    }

    // Non-contextual raw sanity cap.
    //
    // Native P2QR adjusted block weight requires prevouts, so CheckBlock()
    // cannot enforce the final post-P2QR adjusted weight rule here.
    // Keep a larger raw safety cap to prevent absurd physical blocks, then
    // enforce P2QR-adjusted weight contextually when the UTXO view is available.
    if (block.vtx.size() * WITNESS_SCALE_FACTOR > MAX_P2QR_RAW_BLOCK_WEIGHT ||
        ::GetSerializeSize(TX_NO_WITNESS(block)) * WITNESS_SCALE_FACTOR > MAX_P2QR_RAW_BLOCK_WEIGHT) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-length", "raw size limits failed");
    }
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in Kvanta5’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/kvanta5-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // blocks to prevent timewarp attacks (see https://github.com/kvanta5/kvanta5/pull/15482).
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    /*
     * Kvanta5 dev fund creation consensus rule:
     *
     * Block 1 must be the deterministic dev fund block.
     *
     * It must contain exactly one transaction: the coinbase.
     * That coinbase must create exactly 21 outputs of 1,000,000 KV5 each,
     * to the hardcoded native P2QR scripts, in the hardcoded order.
     *
     * Normal 50 KV5 mining subsidy begins at block 2.
     */
    if (nHeight == KVANTA5_DEVFUND_BLOCK_HEIGHT) {
        if (block.vtx.size() != 1) {
            return state.Invalid(
                ...
                "Kvanta5 dev fund block must contain only the dev fund coinbase transaction"
            );
        }

        if (!Kvanta5DevFundCoinbaseIsExpected(*block.vtx[0])) {
            return state.Invalid(
                BlockValidationResult::BLOCK_CONSENSUS,
                "bad-devfund-coinbase",
                "Kvanta5 block 1 coinbase does not match the consensus dev fund allocation"
            );
        }
    }
    ...
    /*
     * Kvanta5 disables SegWit/Taproot/witness paths.
     *
     * Do not require or permit Bitcoin-style witness commitments.
     * Witness commitment outputs are OP_RETURN outputs and are forbidden by
     * Kvanta5 transaction output consensus rules.
     */
     if (!CheckWitnessMalleation(block, false, state)) {
         return false;
    }
    ...
    // failed).
    if (GetBlockWeight(block) > MAX_P2QR_RAW_BLOCK_WEIGHT) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-weight",
                         strprintf("%s : raw weight safety limit failed", __func__));
    }
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/kvanta5-dev/2019-February/016697.html.  Because CheckBlock() is
