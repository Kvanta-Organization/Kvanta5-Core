...
#ifndef KVANTA5_NET_H
#define KVANTA5_NET_H
...
/*
 * Kvanta5 protocol message size limit.
 *
 * Kvanta5 supports larger serialized blocks than Bitcoin. This value must
 * remain comfortably above MAX_BLOCK_SERIALIZED_SIZE so both v1 and BIP324/v2
 * transport can relay full block, cmpctblock, and blocktxn messages without
 * tripping transport-level size rejection.
 *
 * Keep this below absurd memory-abuse territory, but do not make it a razor-thin
 * margin above the consensus block size.
 */
static const unsigned int MAX_PROTOCOL_MESSAGE_LENGTH = 40 * 1000 * 1000;
...
private:
    ...
    // Kvanta5 v2 transport fragmentation.
    //
    // BIP324 uses a 3-byte encrypted packet length field, so one encrypted v2
    // packet cannot carry a full 32 MB Kvanta5 block. Large application messages
    // are split into kv5frag transport-internal messages and reassembled before
    // net_processing receives them.
    struct Kv5V2ReassemblyState {
        bool active{false};
        std::string original_type;
        uint64_t message_id{0};
        uint32_t total_size{0};
        uint16_t fragment_count{0};
        uint16_t next_fragment_index{0};
        uint32_t next_offset{0};
        std::vector<uint8_t> payload;
    };

    Kv5V2ReassemblyState m_kv5_v2_reassembly GUARDED_BY(m_recv_mutex);
    uint64_t m_kv5_v2_send_fragment_id GUARDED_BY(m_send_mutex){0};
...
public:
     ...
     * @see https://github.com/kvanta5/kvanta5/issues/21744 for details.
     */
