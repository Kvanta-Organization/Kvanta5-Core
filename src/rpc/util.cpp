...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 1KV5/kvB are not accepted");
    return CFeeRate{val};
}
...
{
    return "> kvanta5-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> kvanta5-cli -named " + methodname;
...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: application/json' http://127.0.0.1:8333/\n";
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-type: application/json' http://127.0.0.1:8333/\n";
}
...
public:
    ...
    UniValue operator()(const Kvanta5P2QRDestination& id) const
    {
       ...
       uint256 program;
       std::copy(id.begin(), id.end(), program.begin());

       const CScript native_script = GetScriptForDestination(id);
       const CScript redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(program);
       const CScript mining_script = consensus::kvanta5p2qr::MakeP2SHScriptPubKey(program);
       const CScriptID mining_script_id(redeem_script);
       const CTxDestination mining_dest{ScriptHash(mining_script_id)};

       obj.pushKV("isscript", false);
       obj.pushKV("iswitness", false);
       obj.pushKV("iskvanta5p2qr", true);
       obj.pushKV("p2qr_program", HexStr(id));

       obj.pushKV("native_scriptPubKey", HexStr(native_script));

       obj.pushKV("mining_address", EncodeDestination(mining_dest));
       obj.pushKV("mining_type", "p2sh_carried_kvanta5_p2qr");
       obj.pushKV("mining_redeemScript", HexStr(redeem_script));
       obj.pushKV("mining_scriptPubKey", HexStr(mining_script));
    ...
    return
         {
             ...
             {RPCResult::Type::STR, "address", /*optional=*/true, "The Kvanta5 address (only if a well-defined address exists)"},
