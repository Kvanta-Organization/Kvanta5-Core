...
static constexpr std::string_view KV5_V2_FRAGMENT_MSG_TYPE{"kv5frag"};

static constexpr size_t KV5_V2_MAX_BIP324_CONTENTS_LEN{0x00ffffff};
static constexpr size_t KV5_V2_FRAGMENT_DATA_SIZE{8'000'000};
static constexpr size_t KV5_V2_MAX_REASSEMBLED_MESSAGE_SIZE{MAX_PROTOCOL_MESSAGE_LENGTH};
static constexpr size_t KV5_V2_MAX_FRAGMENTS{
    (KV5_V2_MAX_REASSEMBLED_MESSAGE_SIZE + KV5_V2_FRAGMENT_DATA_SIZE - 1) /
    KV5_V2_FRAGMENT_DATA_SIZE
};

static_assert(KV5_V2_MAX_FRAGMENTS == 5);
static_assert(KV5_V2_FRAGMENT_DATA_SIZE < KV5_V2_MAX_BIP324_CONTENTS_LEN);
static_assert(KV5_V2_MAX_REASSEMBLED_MESSAGE_SIZE >= MAX_BLOCK_SERIALIZED_SIZE);

static bool Kv5ReadLE16(Span<const uint8_t>& in, uint16_t& out)
{
...
static bool Kv5ReadLE32(Span<const uint8_t>& in, uint32_t& out)
{
...
static bool Kv5ReadLE64(Span<const uint8_t>& in, uint64_t& out)
{
...
static std::string Kv5FixedToMsgType(Span<const uint8_t> fixed)
{
    size_t n = 0;
    while (n < fixed.size() && fixed[n] != 0) ++n;
    return std::string{reinterpret_cast<const char*>(fixed.data()), n};
}
...
struct Kv5Frag {
    ...
    Span<const uint8_t> fragment_data;
};

static bool Kv5ParseFragmentPayload(Span<const uint8_t> payload, Kv5Frag& frag)
{
    if (payload.size() < CMessageHeader::MESSAGE_TYPE_SIZE + 8 + 4 + 2 + 2 + 4 + 4) {
        return false;
    }

    frag.original_type = Kv5FixedToMsgType(payload.first(CMessageHeader::MESSAGE_TYPE_SIZE));
    payload = payload.subspan(CMessageHeader::MESSAGE_TYPE_SIZE);

    if (!Kv5ReadLE64(payload, frag.message_id)) return false;
    if (!Kv5ReadLE32(payload, frag.total_size)) return false;
    if (!Kv5ReadLE16(payload, frag.fragment_index)) return false;
    if (!Kv5ReadLE16(payload, frag.fragment_count)) return false;
    if (!Kv5ReadLE32(payload, frag.fragment_offset)) return false;
    if (!Kv5ReadLE32(payload, frag.fragment_size)) return false;

    if (payload.size() != frag.fragment_size) return false;

    frag.fragment_data = payload;
    return true;
}

static void Kv5WriteLE16(std::vector<uint8_t>& out, uint16_t v)
{
    out.push_back(uint8_t(v));
    out.push_back(uint8_t(v >> 8));
}

static void Kv5WriteLE32(std::vector<uint8_t>& out, uint32_t v)
{
    out.push_back(uint8_t(v));
    out.push_back(uint8_t(v >> 8));
...
static void Kv5WriteLE64(std::vector<uint8_t>& out, uint64_t v)
{
    for (int i = 0; i < 8; ++i) {
...
static std::array<uint8_t, CMessageHeader::MESSAGE_TYPE_SIZE> Kv5MsgTypeToFixed(std::string_view type)
{
    std::array<uint8_t, CMessageHeader::MESSAGE_TYPE_SIZE> out{};
    const size_t n = std::min(type.size(), out.size());
    std::memcpy(out.data(), type.data(), n);
    return out;
}

static std::vector<uint8_t> Kv5BuildV2LongContents(std::string_view msg_type, Span<const uint8_t> payload)
{
    std::vector<uint8_t> contents;
    contents.reserve(1 + CMessageHeader::MESSAGE_TYPE_SIZE + payload.size());

    // 0x00 = long message type encoding.
    contents.push_back(0x00);

    const auto fixed_type = Kv5MsgTypeToFixed(msg_type);
    contents.insert(contents.end(), fixed_type.begin(), fixed_type.end());
    contents.insert(contents.end(), payload.begin(), payload.end());

    return contents;
}

static std::vector<uint8_t> Kv5BuildFragmentPayload(
    ...
    Span<const uint8_t> fragment_data)
{
    ...
    out.reserve(CMessageHeader::MESSAGE_TYPE_SIZE + 8 + 4 + 2 + 2 + 4 + 4 + fragment_data.size());

    const auto fixed_type = Kv5MsgTypeToFixed(original_type);
    out.insert(out.end(), fixed_type.begin(), fixed_type.end());

    Kv5WriteLE64(out, message_id);
    Kv5WriteLE32(out, total_size);
    Kv5WriteLE16(out, fragment_index);
    Kv5WriteLE16(out, fragment_count);
    Kv5WriteLE32(out, fragment_offset);
    Kv5WriteLE32(out, uint32_t(fragment_data.size()));

    out.insert(out.end(), fragment_data.begin(), fragment_data.end());
    return out;
}
...
static std::optional<std::string> Kv5DecodeV2MessageType(Span<const uint8_t>& contents) noexcept
{
    if (contents.size() == 0) return std::nullopt;

    const uint8_t first_byte = contents[0];
    ...
    while (msg_type_len < CMessageHeader::MESSAGE_TYPE_SIZE) {
        if (contents[msg_type_len] != 0) return std::nullopt;
        ++msg_type_len;
    }
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        if (m_recv_len > MAX_CONTENTS_LEN) {
            LogDebug(BCLog::NET, "Header error: Size too large (%u bytes), peer=%d\n", m_recv_len, m_nodeid);
            LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: v2 packet too large len=%u max_contents=%u peer=%d\n",
                m_recv_len,
                unsigned(MAX_CONTENTS_LEN),
                m_nodeid);
            return false;
        }
        ...
        if (!ignore) {
            ...
            case RecvState::APP:
            {
                // Kvanta5 v2 fragmentation layer.
                //
                // BIP324 has a 3-byte encrypted packet length field, so a full 32 MB Kvanta5
                // block cannot be carried in one v2 packet. Large application messages are
                // carried as transport-internal "kv5frag" packets and reassembled here before
                // net_processing sees them.
                Span<const uint8_t> kv5_contents{m_recv_decode_buffer};
                auto kv5_msg_type = Kv5DecodeV2MessageType(kv5_contents);

                if (kv5_msg_type && *kv5_msg_type == std::string{KV5_V2_FRAGMENT_MSG_TYPE}) {
                    Kv5Frag frag;
                    if (!Kv5ParseFragmentPayload(kv5_contents, frag)) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: malformed fragment peer=%d\n", m_nodeid);
                        return false;
                    }

                    if (frag.original_type.empty() || frag.original_type == std::string{KV5_V2_FRAGMENT_MSG_TYPE}) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: invalid original type type=%s peer=%d\n",
                            frag.original_type,
                            m_nodeid);
                        return false;
                    }

                    if (frag.total_size == 0 || frag.total_size > KV5_V2_MAX_REASSEMBLED_MESSAGE_SIZE) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: bad total_size=%u max=%u peer=%d\n",
                            frag.total_size,
                            unsigned(KV5_V2_MAX_REASSEMBLED_MESSAGE_SIZE),
                            m_nodeid);
                        return false;
                    }

                    if (frag.fragment_count == 0 || frag.fragment_count > KV5_V2_MAX_FRAGMENTS) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: bad fragment_count=%u max=%u peer=%d\n",
                            frag.fragment_count,
                            unsigned(KV5_V2_MAX_FRAGMENTS),
                            m_nodeid);
                        return false;
                    }

                    if (frag.fragment_size == 0 || frag.fragment_size > KV5_V2_FRAGMENT_DATA_SIZE) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: bad fragment_size=%u max=%u peer=%d\n",
                            frag.fragment_size,
                            unsigned(KV5_V2_FRAGMENT_DATA_SIZE),
                            m_nodeid);
                        return false;
                    }

                    if (frag.fragment_index >= frag.fragment_count) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: index out of range index=%u count=%u peer=%d\n",
                            frag.fragment_index,
                            frag.fragment_count,
                            m_nodeid);
                        return false;
                    ...
                    const uint32_t expected_offset = uint32_t(size_t(frag.fragment_index) * KV5_V2_FRAGMENT_DATA_SIZE);

                    if (frag.fragment_offset != expected_offset) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: bad offset got=%u expected=%u index=%u peer=%d\n",
                    ...
                    if (uint64_t(frag.fragment_offset) + uint64_t(frag.fragment_size) > uint64_t(frag.total_size)) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: fragment exceeds total offset=%u size=%u total=%u peer=%d\n",
                            frag.fragment_offset,
                            frag.fragment_size,
                            frag.total_size,
                            m_nodeid);
                    ...
                    const bool is_last = frag.fragment_index + 1 == frag.fragment_count;
                    if (!is_last && frag.fragment_size != KV5_V2_FRAGMENT_DATA_SIZE) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: non-last fragment wrong size index=%u size=%u expected=%u peer=%d\n",
                            frag.fragment_index,
                            frag.fragment_size,
                            unsigned(KV5_V2_FRAGMENT_DATA_SIZE),
                            m_nodeid);
                    ...
                    if (!m_kv5_v2_reassembly.active) {
                        if (frag.fragment_index != 0 || frag.fragment_offset != 0) {
                            LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: first fragment not index zero index=%u offset=%u peer=%d\n",
                                frag.fragment_index,
                                frag.fragment_offset,
                                m_nodeid);
                            return false;
                        ...
                        m_kv5_v2_reassembly.active = true;
                        m_kv5_v2_reassembly.original_type = frag.original_type;
                        m_kv5_v2_reassembly.message_id = frag.message_id;
                        m_kv5_v2_reassembly.total_size = frag.total_size;
                        m_kv5_v2_reassembly.fragment_count = frag.fragment_count;
                        m_kv5_v2_reassembly.next_fragment_index = 0;
                        m_kv5_v2_reassembly.next_offset = 0;
                        m_kv5_v2_reassembly.payload.clear();
                        m_kv5_v2_reassembly.payload.reserve(frag.total_size);

                        LogPrintf("KVANTA5_V2_FRAGMENT: starting reassembly type=%s total=%u fragments=%u peer=%d\n",
                            frag.original_type,
                            frag.total_size,
                            frag.fragment_count,
                            m_nodeid);
                    ...
                    auto& r = m_kv5_v2_reassembly;

                    if (frag.message_id != r.message_id ||
                        frag.original_type != r.original_type ||
                        frag.total_size != r.total_size ||
                        frag.fragment_count != r.fragment_count ||
                        frag.fragment_index != r.next_fragment_index ||
                        frag.fragment_offset != r.next_offset) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: unexpected fragment id=%u index=%u expected_index=%u offset=%u expected_offset=%u peer=%d\n",
                            unsigned(frag.message_id),
                            frag.fragment_index,
                            r.next_fragment_index,
                            frag.fragment_offset,
                            r.next_offset,
                            m_nodeid);
                        return false;
                    ...
                    r.payload.insert(r.payload.end(), frag.fragment_data.begin(), frag.fragment_data.end());
                    r.next_offset += frag.fragment_size;
                    r.next_fragment_index += 1;

                    // This decrypted packet was an internal transport fragment.
                    // If reassembly is incomplete, consume it without emitting an application message.
                    if (r.next_offset < r.total_size) {
                        ClearShrink(m_recv_buffer);
                    ...
                    if (r.next_offset != r.total_size || r.next_fragment_index != r.fragment_count) {
                        LogPrintf("KVANTA5_V2_FRAGMENT_ERROR: final size mismatch got=%u total=%u index=%u count=%u peer=%d\n",
                            r.next_offset,
                            r.total_size,
                            r.next_fragment_index,
                            r.fragment_count,
                    ...
                    LogPrintf("KVANTA5_V2_FRAGMENT: reassembled message type=%s total=%u fragments=%u peer=%d\n",
                        r.original_type,
                        unsigned(r.payload.size()),
                        r.fragment_count,
                        m_nodeid);

                    // Replace decrypted contents with a normal long-encoded v2 application message.
                    // GetReceivedMessage() will now emit this as if it arrived normally.
                    m_recv_decode_buffer = Kv5BuildV2LongContents(
                        r.original_type,
                        Span<const uint8_t>{r.payload}
                    );

                    m_recv_len = m_recv_decode_buffer.size();
                    m_kv5_v2_reassembly = Kv5V2ReassemblyState{};
                }

                // Application message decrypted correctly. It can be extracted using GetMessage().
                // For a normal packet, this emits the normal message.
                // For a completed kv5frag sequence, this emits the reassembled original message.
...
bool V2Transport::SetMessageToSend(CSerializedNetMsg& msg) noexcept
{
    ...
    if (m_send_state == SendState::V1) {
        return m_v1_fallback.SetMessageToSend(msg);
    }

    // We only allow adding a new message to be sent when in the READY state
    // and the send buffer is empty. This limits the send buffer to one
    // application message. For fragmented Kvanta5 v2 messages, all encrypted
    // fragments for that one application message are queued together.
    if (!(m_send_state == SendState::READY && m_send_buffer.empty())) {
        return false;
    }

    // Construct normal v2 contents: message type encoding + payload.
    std::vector<uint8_t> contents;
    ...
    if (short_message_id) {
        ...
        // Long message type encoding:
        // contents[0] = 0x00
        // contents[1..12] = null-padded message type
        // contents[13..] = payload
    ...
    // Normal path: fits in one BIP324 packet.
    if (contents.size() <= KV5_V2_MAX_BIP324_CONTENTS_LEN) {
    ...
    // Kvanta5 large-message path:
    // BIP324 uses a 3-byte packet length, so full 32 MB Kvanta5 blocks must be
    // fragmented into multiple v2 packets and reassembled before net_processing.
    const size_t payload_size = msg.data.size();

    if (payload_size > KV5_V2_MAX_REASSEMBLED_MESSAGE_SIZE) {
        LogPrintf(
            "KVANTA5_V2_FRAGMENT_ERROR: refusing oversized message type=%s payload=%u max=%u peer=%d\n",
            msg.m_type,
    ...
    const uint16_t fragment_count = uint16_t(
        (payload_size + KV5_V2_FRAGMENT_DATA_SIZE - 1) / KV5_V2_FRAGMENT_DATA_SIZE
    );

    if (fragment_count == 0 || fragment_count > KV5_V2_MAX_FRAGMENTS) {
        LogPrintf(
            "KVANTA5_V2_FRAGMENT_ERROR: invalid fragment count type=%s payload=%u fragments=%u max=%u peer=%d\n",
            msg.m_type,
            unsigned(payload_size),
    ...
    const uint64_t message_id = ++m_kv5_v2_send_fragment_id;

    LogPrintf(
        "KVANTA5_V2_FRAGMENT: sending fragmented message type=%s payload=%u fragments=%u chunk=%u peer=%d\n",
        msg.m_type,
        unsigned(payload_size),
        unsigned(fragment_count),
        unsigned(KV5_V2_FRAGMENT_DATA_SIZE),
    ...
    for (uint16_t index = 0; index < fragment_count; ++index) {
        const uint32_t offset = uint32_t(size_t(index) * KV5_V2_FRAGMENT_DATA_SIZE);
        const uint32_t frag_size = uint32_t(std::min<size_t>(
            KV5_V2_FRAGMENT_DATA_SIZE,
            payload_size - offset
        ));

        const auto frag_payload = Kv5BuildFragmentPayload(
            msg.m_type,
            message_id,
            ...
            fragment_count,
            offset,
            Span<const uint8_t>{msg.data}.subspan(offset, frag_size)
        );

        const auto frag_contents = Kv5BuildV2LongContents(
            KV5_V2_FRAGMENT_MSG_TYPE,
            Span<const uint8_t>{frag_payload}
        );

        if (frag_contents.size() > KV5_V2_MAX_BIP324_CONTENTS_LEN) {
            LogPrintf(
                "KVANTA5_V2_FRAGMENT_ERROR: fragment contents too large type=%s index=%u contents=%u max=%u peer=%d\n",
                msg.m_type,
                unsigned(index),
                unsigned(frag_contents.size()),
                unsigned(KV5_V2_MAX_BIP324_CONTENTS_LEN),
                m_nodeid);
            ClearShrink(m_send_buffer);
            return false;
        }

        const size_t old_size = m_send_buffer.size();
        m_send_buffer.resize(old_size + frag_contents.size() + BIP324Cipher::EXPANSION);

        m_cipher.Encrypt(
            MakeByteSpan(frag_contents),
            {},
            false,
            MakeWritableByteSpan(m_send_buffer).subspan(old_size)
        );
    }
