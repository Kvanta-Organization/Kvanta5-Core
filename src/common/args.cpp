...
const char * const KVANTA5_CONF_FILENAME = "kvanta5.conf";
const char * const KVANTA5_SETTINGS_FILENAME = "settings.json";
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ...
        ChainTypeToString(ChainType::TESTNET5),
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //kvanta5-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", KVANTA5_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Kvanta5
    //   new: C:\Users\Username\AppData\Local\Kvanta5
    // macOS: ~/Library/Application Support/Kvanta5
    // Unix-like: ~/.kvanta5
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Kvanta5";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Kvanta5";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Kvanta5";
#else
    // Unix-like
    return pathRet / ".kvanta5";
#endif
#endif
}
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fTestNet5 > 1) {
    throw std::runtime_error("Invalid combination of -regtest, -signet, -testnet5 and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fTestNet5) return ChainType::TESTNET5;
