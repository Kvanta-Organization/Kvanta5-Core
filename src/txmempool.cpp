...
CTxMemPool::ChangeSet::TxHandle CTxMemPool::ChangeSet::StageAddition(
    ...
    int64_t sigops_cost,
    LockPoints lp,
    int64_t p2qr_adjusted_tx_weight)
{
    LOCK(m_pool->cs);

    Assume(m_to_add.find(tx->GetHash()) == m_to_add.end());

    auto [newit, inserted] = m_to_add.emplace(
        tx,
        fee,
        time,
        entry_height,
    ...
    if (delta) {
        m_to_add.modify(newit, [&delta](CTxMemPoolEntry& e) {
