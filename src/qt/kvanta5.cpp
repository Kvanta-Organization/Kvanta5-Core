...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<Kvanta5Unit>("Kvanta5Unit");
#else
    qRegisterMetaType<Kvanta5Unit>("Kvanta5Unit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. kvanta5_de.qm (shortcut "de" needs to be defined in kvanta5.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. kvanta5_de_DE.qm (shortcut "de_DE" needs to be defined in kvanta5.qrc)
...
static const char* qt_argv = "kvanta5-qt";

Kvanta5Application::Kvanta5Application()
...
void Kvanta5Application::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the Kvanta5Application constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", Kvanta5GUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void Kvanta5Application::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool Kvanta5Application::createOptionsModel(bool resetSettings)
{
...
void Kvanta5Application::createWindow(const NetworkStyle *networkStyle)
{
    window = new Kvanta5GUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &Kvanta5GUI::quitRequested, this, &Kvanta5Application::requestShutdown);
...
void Kvanta5Application::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void Kvanta5Application::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool Kvanta5Application::baseInitialize()
{
    return node().baseInitialize();
}

void Kvanta5Application::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &Kvanta5Application::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &Kvanta5Application::handleRunawayException);
    connect(this, &Kvanta5Application::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &Kvanta5Application::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void Kvanta5Application::parameterSetup()
{
...
void Kvanta5Application::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void Kvanta5Application::requestInitialize()
{
...
void Kvanta5Application::requestShutdown()
{
...
void Kvanta5Application::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // kvanta5: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &Kvanta5GUI::handlePaymentRequest);
            connect(window, &Kvanta5GUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void Kvanta5Application::handleRunawayException(const QString &message)
{
...
void Kvanta5Application::handleNonFatalException(const QString& message)
{
...
WId Kvanta5Application::getMainWinId() const
{
    if (!window)
        return 0;
...
bool Kvanta5Application::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", Kvanta5GUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(kvanta5);
    Q_INIT_RESOURCE(kvanta5_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(KVANTA5_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see kvanta5-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see kvanta5-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse kvanta5.conf, determine network, switch to network specific
    ...
    // kvanta5: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
