    ...
    : QWidget(parent), m_platform_style{platformStyle}
{
    ...
    /*
     * P2QR transaction display needs full, non-elided two-line addresses.
     *
     * Keep these display/layout settings in the constructor because they do not
     * depend on the transaction model already being attached.
     *
     * Do NOT configure per-column widths or per-column resize modes here.
     * During mainnet wallet creation the TransactionView can be constructed
     * before transactionProxyModel is installed. Calling QHeaderView per-section
     * methods before the header has a valid section count can crash inside Qt.
     * Column-specific setup is applied safely in setModel().
     */
    transactionView->setWordWrap(true);
    transactionView->setTextElideMode(Qt::ElideNone);

    transactionView->verticalHeader()->setDefaultSectionSize(64);
    transactionView->verticalHeader()->setMinimumSectionSize(64);
    ...
    copyP2QRMiningAddressAction = contextMenu->addAction(tr("Copy &P2QR Mining Address"), this, &TransactionView::copyP2QRMiningAddress);
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if (_model)
    {
        ...
        /*
         * Apply transaction table column layout only after the model is attached.
         *
         * Mainnet wallet creation can construct TransactionView before the model
         * has any sections. Calling QHeaderView::setSectionResizeMode(column, ...)
         * too early can segfault inside QHeaderViewPrivate::createSectionItems().
         *
         * The guards below preserve the P2QR two-line address layout while making
         * the column setup safe for mainnet, testnet5, fresh wallets, restored
         * wallets, and weird saved GUI state.
         */
        QHeaderView* header = transactionView->horizontalHeader();

        auto safeSetColumnWidth = [&](int column, int width) {
            const int columns = transactionView->model() ? transactionView->model()->columnCount() : 0;
            if (column >= 0 && column < columns) {
                transactionView->setColumnWidth(column, width);
            } else {
                qWarning() << "TransactionView: skipping invalid column width"
                           << column << "column_count" << columns;
            }
        };

        auto safeSetSectionResizeMode = [&](int column, QHeaderView::ResizeMode mode) {
            const int columns = transactionView->model() ? transactionView->model()->columnCount() : 0;
            if (column >= 0 && column < columns) {
                header->setSectionResizeMode(column, mode);
            } else {
                qWarning() << "TransactionView: skipping invalid section resize"
                           << column << "column_count" << columns;
            }
        };
        ...
        if (!header->restoreState(settings.value("TransactionViewHeaderState").toByteArray())) {
            safeSetColumnWidth(TransactionTableModel::Status, STATUS_COLUMN_WIDTH);
            safeSetColumnWidth(TransactionTableModel::Watchonly, WATCHONLY_COLUMN_WIDTH);
            safeSetColumnWidth(TransactionTableModel::Date, DATE_COLUMN_WIDTH);
            safeSetColumnWidth(TransactionTableModel::Type, TYPE_COLUMN_WIDTH);
            safeSetColumnWidth(TransactionTableModel::Amount, AMOUNT_MINIMUM_COLUMN_WIDTH);
        }

        header->setStretchLastSection(false);
        header->setMinimumSectionSize(MINIMUM_COLUMN_WIDTH);

        safeSetSectionResizeMode(TransactionTableModel::Status, QHeaderView::Fixed);
        safeSetSectionResizeMode(TransactionTableModel::Watchonly, QHeaderView::Fixed);
        safeSetSectionResizeMode(TransactionTableModel::Date, QHeaderView::Fixed);
        safeSetSectionResizeMode(TransactionTableModel::Type, QHeaderView::Fixed);
        safeSetSectionResizeMode(TransactionTableModel::Amount, QHeaderView::ResizeToContents);
        safeSetSectionResizeMode(TransactionTableModel::ToAddress, QHeaderView::Stretch);

        transactionView->resizeRowsToContents();
        ...
        if (_model->getOptionsModel())
        {
            ...
            for (int i = 0; i < listUrls.size(); ++i)
            {
                ...
                if (!host.isEmpty())
                {
                    ...
                    contextMenu->addAction(tr("Show in %1").arg(host), [this, url] {
...
void TransactionView::changedAmount()
{
    ...
    if (Kvanta5Units::parse(model->getOptionsModel()->getDisplayUnit(), amountWidget->text(), &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(Kvanta5Units::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::copyP2QRMiningAddress()
{
    GUIUtil::copyEntryData(transactionView, 0, TransactionTableModel::P2QRMiningAddressRole);
}
