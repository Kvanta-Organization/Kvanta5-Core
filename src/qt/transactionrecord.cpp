    ...
    if (fAllFromMe || !any_from_me) {
        ...
        for(unsigned int i = 0; i < wtx.tx->vout.size(); i++)
        {
            ...
            if (fAllFromMe) {
                ...
                // Otherwise, someone just sent kvanta5s to a change address, which should be shown
                ...
                {
                    // Sent to Kvanta5 Address
                    sub.type = TransactionRecord::SendToAddress;
                    sub.address = EncodeDestination(wtx.txout_address[i]);
                    if (i < wtx.txout_p2qr_mining_address.size()) {
                        sub.p2qr_mining_address = wtx.txout_p2qr_mining_address[i];
                    }
                }
                else
                {
