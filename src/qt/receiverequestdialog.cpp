...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatKvanta5URI(info);
    ...
    if (!info.p2qrMiningAddress.isEmpty()) {
    ui->p2qr_mining_address_content->setText(info.p2qrMiningAddress);
    } else {
    ui->p2qr_mining_address_tag->hide();
    ui->p2qr_mining_address_content->hide();
    }
...
{
    if (!model) return;
    ui->amount_content->setText(Kvanta5Units::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatKvanta5URI(info));
}
