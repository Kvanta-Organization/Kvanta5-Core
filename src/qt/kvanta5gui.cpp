...
const std::string Kvanta5GUI::DEFAULT_UIPLATFORM =
...
Kvanta5GUI::Kvanta5GUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &Kvanta5GUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &Kvanta5GUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &Kvanta5GUI::showModalOverlay);
...
void Kvanta5GUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kvanta5 address"));
    sendCoinsAction->setToolTip(sendCoinsAction->statusTip());
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(QStringLiteral("Alt+2")));
    tabGroup->addAction(sendCoinsAction);
    
    vaultsAction = new QAction(platformStyle->SingleColorIcon(":/icons/lock_closed"), tr("&Vaults"), this);
    vaultsAction->setStatusTip(tr("Create, approve, and broadcast Kvanta5 multi-signature vault payments"));
    vaultsAction->setToolTip(vaultsAction->statusTip());
    vaultsAction->setCheckable(true);
    vaultsAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
    tabGroup->addAction(vaultsAction);
    
    
    receiveCoinsAction = new QAction(platformStyle->SingleColorIcon(":/icons/receiving_addresses"), tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kvanta5: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &Kvanta5GUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(vaultsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(vaultsAction, &QAction::triggered, [this]{ gotoVaultsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &Kvanta5GUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &Kvanta5GUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kvanta5 addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kvanta5 addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Kvanta5 Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Kvanta5 Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a kvanta5: URI"));
    ...
    connect(quitAction, &QAction::triggered, this, &Kvanta5GUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &Kvanta5GUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &Kvanta5GUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &Kvanta5GUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &Kvanta5GUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &Kvanta5GUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &Kvanta5GUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &Kvanta5GUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &Kvanta5GUI::createWallet);
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &Kvanta5GUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &Kvanta5GUI::showDebugWindow);
}

void Kvanta5GUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void Kvanta5GUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        toolbar->addAction(vaultsAction);
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &Kvanta5GUI::setCurrentWalletBySelectorIndex);
...
void Kvanta5GUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &Kvanta5GUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &Kvanta5GUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &Kvanta5GUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &Kvanta5GUI::showProgress);
...
#ifdef ENABLE_WALLET
void Kvanta5GUI::enableHistoryAction(bool privacy)
{
...
void Kvanta5GUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
...
WalletController* Kvanta5GUI::getWalletController()
{
    return m_wallet_controller;
}

void Kvanta5GUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &Kvanta5GUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &Kvanta5GUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &Kvanta5GUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &Kvanta5GUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &Kvanta5GUI::incomingTransaction);
    connect(this, &Kvanta5GUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void Kvanta5GUI::removeWallet(WalletModel* walletModel)
{
...
void Kvanta5GUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void Kvanta5GUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void Kvanta5GUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void Kvanta5GUI::setWalletActionsEnabled(bool enabled)
{
    overviewAction->setEnabled(enabled);
    sendCoinsAction->setEnabled(enabled);
    vaultsAction->setEnabled(enabled);
...
void Kvanta5GUI::createTrayIcon()
{
...
void Kvanta5GUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &Kvanta5GUI::toggleHidden);
...
void Kvanta5GUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void Kvanta5GUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void Kvanta5GUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void Kvanta5GUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void Kvanta5GUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void Kvanta5GUI::openClicked()
{
...
void Kvanta5GUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void Kvanta5GUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void Kvanta5GUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void Kvanta5GUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void Kvanta5GUI::gotoVaultsPage()
{
    if (walletFrame) {
        walletFrame->gotoVaultsPage();
    }
}


void Kvanta5GUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void Kvanta5GUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void Kvanta5GUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void Kvanta5GUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Kvanta5 network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void Kvanta5GUI::setNumConnections(int count)
{
    updateNetworkState();
}

void Kvanta5GUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void Kvanta5GUI::updateHeadersSyncProgressLabel()
{
...
void Kvanta5GUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void Kvanta5GUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &Kvanta5GUI::quitRequested);
...
void Kvanta5GUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void Kvanta5GUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &Kvanta5GUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void Kvanta5GUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void Kvanta5GUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &Kvanta5GUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &Kvanta5GUI::show);
                e->ignore();
            }
        }
    }
...
void Kvanta5GUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void Kvanta5GUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void Kvanta5GUI::incomingTransaction(const QString& date, Kvanta5Unit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(Kvanta5Units::formatWithUnit(unit, amount, true));
...
void Kvanta5GUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void Kvanta5GUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool Kvanta5GUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool Kvanta5GUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void Kvanta5GUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void Kvanta5GUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void Kvanta5GUI::updateWalletStatus()
{
    assert(walletFrame);
...
void Kvanta5GUI::updateProxyIcon()
{
    std::string ip_port;
...
void Kvanta5GUI::updateWindowTitle()
{
...
void Kvanta5GUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void Kvanta5GUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void Kvanta5GUI::detectShutdown()
{
...
void Kvanta5GUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void Kvanta5GUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(Kvanta5GUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = Kvanta5GUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void Kvanta5GUI::subscribeToCoreSignals()
{
...
void Kvanta5GUI::unsubscribeFromCoreSignals()
{
...
bool Kvanta5GUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<Kvanta5Unit> units = Kvanta5Units::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const Kvanta5Unit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, Kvanta5Units::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const Kvanta5Unit u : Kvanta5Units::availableUnits()) {
        menu->addAction(Kvanta5Units::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(Kvanta5Unit newUnits)
{
    setText(Kvanta5Units::longName(newUnits));
}
