This directory contains the source code for the Kvanta5 Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Kvanta5 UI.
...
- ClientModel is used by the main application `kvanta5gui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a kvanta5:... link from a browser).
...
* UI elements like Kvanta5AmountField, which inherit from QWidget.
* `kvanta5strings.cpp`: automatically generated
* `kvanta5units.(h/cpp)`: KV5 / mKV5 / etc. handling
* `callback.h`
...
**Note:** Do not change `local/kvanta5_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `kvanta5-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `kvanta5-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `kvanta5-qt`
9. You're all set! Start developing, building, and debugging the Kvanta5 Core GUI
