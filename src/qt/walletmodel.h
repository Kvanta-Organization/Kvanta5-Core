...
#ifndef KVANTA5_QT_WALLETMODEL_H
#define KVANTA5_QT_WALLETMODEL_H
...
/** Interface to Kvanta5 wallet from Qt view code. */
...
public:
    ...
    QString getP2QRMiningAddress(const QString& p2qrAddress) const;
    
    interfaces::Kvanta5P2QRSignerInfo createP2QRSigner(
        const QString& label,
        QString& error) const;

    interfaces::Kvanta5P2QRMultisigInfo createP2QRMultisig(
        int required,
    ...
    QString createVaultTxPackage(
        const QList<SendCoinsRecipient>& recipients,
        const wallet::CCoinControl& coin_control,
        QString& error) const;

    QString approveVaultTxPackage(
        const QString& package_json,
        QString& error) const;

    QString finalizeVaultTxPackage(
        const QString& package_json,
        QString& error) const;
    
    QString broadcastTransactionHex(
    ...
    struct VaultUTXO
    {
        ...
        QString address;
        QString program;
        QString type; // "Native P2QR" / "Wrapped P2SH"
        CAmount amount{0};
        int confirmations{0};
    };
    
    struct VaultSignerInfo
    {
        int signer_index{0};
        QString pubkey;
        QString address;
    ...
    struct VaultInfo
    {
        ...
        int utxos{0};
        int spend_count{0};
        QVector<VaultSignerInfo> signers;
    };
    
    QVector<VaultInfo> listVaults() const;
    QVector<VaultUTXO> listVaultUTXOs() const;
    QString dumpKvanta5VaultBackup(const QString& address, const QString& filename, QString& error);
    QString importKvanta5VaultBackup(const QString& filename, const QString& label, QString& error);
...
#endif // KVANTA5_QT_WALLETMODEL_H
