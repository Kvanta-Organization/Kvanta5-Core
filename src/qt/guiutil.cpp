...
#include <qt/kvanta5addressvalidator.h>
...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::MAIN:
        addr = "kvqr1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tq2jku9f";
        break;
    case ChainType::SIGNET:
    case ChainType::TESTNET5:
        addr = "tnqr1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqa6qnlg";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Kvanta5 address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new Kvanta5AddressEntryValidator(parent));
    widget->setCheckValidator(new Kvanta5AddressCheckValidator(parent));
}
...
bool parseKvanta5URI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no kvanta5: URI
    if(!uri.isValid() || uri.scheme() != QString("kvanta5"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!Kvanta5Units::parse(Kvanta5Unit::KV5, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseKvanta5URI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseKvanta5URI(uriInstance, out);
}

QString formatKvanta5URI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("kvanta5:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(Kvanta5Units::format(Kvanta5Unit::KV5, info.amount, false, Kvanta5Units::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openKvanta5Conf()
{
    ...
    /* Open kvanta5.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kvanta5.lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Kvanta5 (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Kvanta5*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "kvanta5.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("kvanta5-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a kvanta5.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Kvanta5\n";
        else
            optionFile << strprintf("Name=Kvanta5 (%s)\n", ChainTypeToString(chain));
