...
interfaces::Kvanta5P2QRSignerInfo WalletModel::createP2QRSigner(
    const QString& label,
    QString& error) const
{
    auto result = wallet().createKvanta5P2QRSigner(label.toStdString());
    if (!result) {
...
interfaces::Kvanta5P2QRMultisigInfo WalletModel::createP2QRMultisig(
    int required,
    ...
    QString& error) const
{
    ...
    util::Result<interfaces::Kvanta5P2QRMultisigInfo> result =
        wallet().createKvanta5P2QRMultisig(
            required,
    ...
    QString& error) const
{
    ...
    auto result = wallet().createKvanta5VaultTxPackage(vec_send, coin_control);
    if (!result) {
        error = QString::fromStdString(util::ErrorString(result).translated);
        return {};
    }
...
QString WalletModel::approveVaultTxPackage(
    ...
    QString& error) const
{
    auto result = wallet().approveKvanta5VaultTxPackage(package_json.toStdString());
    if (!result) {
        error = QString::fromStdString(util::ErrorString(result).translated);
        return {};
    }
...
QString WalletModel::finalizeVaultTxPackage(
    ...
    QString& error) const
{
    auto result = wallet().finalizeKvanta5VaultTxPackage(package_json.toStdString());
    if (!result) {
        error = QString::fromStdString(util::ErrorString(result).translated);
        return {};
    }
...
QVector<WalletModel::VaultUTXO> WalletModel::listVaultUTXOs() const
{
    ...
    const std::vector<interfaces::Kvanta5VaultUTXO> backend_utxos =
        m_wallet->listKvanta5VaultUTXOs();

    result.reserve(static_cast<int>(backend_utxos.size()));

    for (const interfaces::Kvanta5VaultUTXO& backend_utxo : backend_utxos) {
...
QVector<WalletModel::VaultInfo> WalletModel::listVaults() const
{
    ...
    const std::vector<interfaces::Kvanta5VaultInfo> backend_vaults =
        m_wallet->listKvanta5Vaults();

    result.reserve(static_cast<int>(backend_vaults.size()));

    for (const interfaces::Kvanta5VaultInfo& backend_vault : backend_vaults) {
        ...
        for (const interfaces::Kvanta5VaultSignerInfo& backend_signer : backend_vault.signers) {
...
QString WalletModel::dumpKvanta5VaultBackup(const QString& address, const QString& filename, QString& error)
{
    error.clear();

    util::Result<std::string> result = wallet().dumpKvanta5VaultBackup(
        address.toStdString(),
...
QString WalletModel::importKvanta5VaultBackup(const QString& filename, const QString& label, QString& error)
{
    error.clear();

    util::Result<std::string> result = wallet().importKvanta5VaultBackup(
        filename.toStdString(),
        label.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered kvanta5 address / amount:
...
void WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        {
            if (!rcp.message.isEmpty()) // Message from normal kvanta5:URI (kvanta5:123...?message=example)
...
bool WalletModel::bumpFee(uint256 hash, uint256& new_hash)
{
    ...
    questionString.append(Kvanta5Units::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(Kvanta5Units::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(Kvanta5Units::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));
    ...
    const bool always_show_unsigned{getOptionsModel()->getEnablePSKV5ontrols()};
