...
/** Call node.updateRwSetting() with Kvanta5 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Kvanta5 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/kvanta5/kvanta5/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because kvanta5 22.x will not try to read these.
...
//! Convert enabled/size values to kvanta5 -prune setting.
...
//! Get pruning enabled value to show in GUI from kvanta5 -prune setting.
...
//! Get pruning size value to show in GUI from kvanta5 -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayKvanta5Unit")) {
        settings.setValue("DisplayKvanta5Unit", QVariant::fromValue(Kvanta5Unit::KV5));
    }
    QVariant unit = settings.value("DisplayKvanta5Unit");
    if (unit.canConvert<Kvanta5Unit>()) {
        m_display_kvanta5_unit = unit.value<Kvanta5Unit>();
    } else {
        m_display_kvanta5_unit = Kvanta5Unit::KV5;
        settings.setValue("DisplayKvanta5Unit", QVariant::fromValue(m_display_kvanta5_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if kvanta5.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_kvanta5_unit);
...
{
    if (new_unit.isNull() || new_unit.value<Kvanta5Unit>() == m_display_kvanta5_unit) return;
    m_display_kvanta5_unit = new_unit.value<Kvanta5Unit>();
    QSettings settings;
    settings.setValue("DisplayKvanta5Unit", QVariant::fromValue(m_display_kvanta5_unit));
    Q_EMIT displayUnitChanged(m_display_kvanta5_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/kvanta5-core/gui/issues/567).
