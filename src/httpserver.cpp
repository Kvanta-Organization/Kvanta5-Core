...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    // and https://github.com/kvanta5/kvanta5/pull/11593.
...
bool InitHTTPServer(const util::SignalInterrupt& interrupt)
{
    ...
    // submitblock sends block hex inside JSON, so a 32 MB raw block can
    // require roughly 64 MB+ of HTTP request body. Keep this RPC-only cap
    // separate from consensus/P2P message limits.
    static constexpr uint64_t MAX_RPC_HTTP_BODY_SIZE = 128ULL * 1024ULL * 1024ULL;
    evhttp_set_max_body_size(http, MAX_RPC_HTTP_BODY_SIZE);
