...
#ifndef KVANTA5_WALLET_WALLET_H
#define KVANTA5_WALLET_WALLET_H
...
//! -fallbackfee default
//
// Kvanta5 early-network safety:
//
// Smart fee estimation remains the primary path when fee data exists.
// But on a young/quiet chain, the estimator may have insufficient data.
// A nonzero fallback keeps normal wallet transactions from getting stranded
// or requiring manual fee bumping during early miner release.
//
// 10000 quarks/kvB = 0.00010000 KV5/kvB.
...
struct Kvanta5P2QRSpendInfo
{
    ...
    bool multisig{false};
    uint16_t multisig_required{0};
    std::vector<uint256> multisig_signer_programs;
};

struct Kvanta5VaultApproval
{
...
struct Kvanta5VaultTxPackage
{
    CMutableTransaction tx;
    std::vector<Kvanta5VaultInput> inputs;
    std::vector<Kvanta5VaultApproval> approvals;

    std::string vault_label;
    Kvanta5P2QRDestination vault_dest;
    uint256 vault_program;
    uint16_t required{0};
...
public:
     ...
     * kvanta5-wallet tool code or by wallet migration code. If this is called
    ...
    /** Seed-backed native KV5 P2QR keys, indexed by HASH256(ML-DSA-87 pubkey). */
    std::unique_ptr<Kvanta5P2QRScriptPubKeyMan> m_kvanta5_p2qr_spk_man GUARDED_BY(cs_wallet);
    
    
    std::optional<Kvanta5P2QRSpendInfo> GetKvanta5P2QRSpendInfoForScript(const CScript& script) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    
    bool LoadKvanta5P2QRMultisig(const uint256& program, const Kvanta5P2QRMultisigRecord& record) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<Kvanta5P2QRDestination> AddKvanta5P2QRMultisig(
       uint16_t required,
       std::vector<uint256> signer_programs,
       const std::string& label) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    
    util::Result<Kvanta5VaultTxPackage> CreateKvanta5VaultTxPackage(
        const std::vector<CRecipient>& recipients,
        const CCoinControl& coin_control) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<Kvanta5VaultTxPackage> ApproveKvanta5VaultTxPackage(
        const Kvanta5VaultTxPackage& package) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<CTransactionRef> FinalizeKvanta5VaultTxPackage(
        const Kvanta5VaultTxPackage& package) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
        
    util::Result<std::string> EncodeKvanta5VaultTxPackage(
        const Kvanta5VaultTxPackage& package) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<Kvanta5VaultTxPackage> DecodeKvanta5VaultTxPackage(
        const std::string& json) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    util::Result<CScript> GetNewKvanta5P2QRChangeScript();
    util::Result<CScript> GetNewKvanta5P2QRCompatibilityChangeScript();
    util::Result<Kvanta5P2QRDestination> GenerateNewKvanta5P2QRDestination(const std::string& label);
    bool ImportKvanta5P2QRSeed(Span<const unsigned char> seed, const std::string& label, Kvanta5P2QRDestination& dest) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetKvanta5P2QRSeed(const Kvanta5P2QRDestination& dest, std::vector<unsigned char>& seed_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetKvanta5P2QRKey(const Kvanta5P2QRDestination& dest, CPQKey& key_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool HaveKvanta5P2QRDestination(const Kvanta5P2QRDestination& dest) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetKvanta5P2QRPubKey(const Kvanta5P2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    //! Kvanta5P2QR Script PubKeyMan.
    Kvanta5P2QRScriptPubKeyMan* GetKvanta5P2QRScriptPubKeyMan() const;
    Kvanta5P2QRScriptPubKeyMan* GetOrCreateKvanta5P2QRScriptPubKeyMan();
    bool GetKvanta5P2QRDestinationForScript(const CScript& script, Kvanta5P2QRDestination& dest) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
