...
util::Result<CTxDestination> Kvanta5P2QRScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    CPQKey key;
    if (!key.MakeNewKey()) {
    ...
    if (!AddKey(batch, key_hash, record)) {
        return util::Error{_("Error: Failed to write KV5 P2QR key to wallet database.")};
    }

    return CTxDestination{dest};
}

isminetype Kvanta5P2QRScriptPubKeyMan::IsMine(const CScript& script) const
{
    uint256 program;
    ...
    if (consensus::kvanta5p2qr::IsOutput(script, program)) {
        ...
        if (m_keys.count(program) != 0) {
            return ISMINE_SPENDABLE;
        }

        if (m_multisig.count(program) != 0) {
            return ISMINE_SPENDABLE;
        }
    ...
    // P2SH-carried P2QR compatibility container:
    //   a9 14 HASH160(50 20 <program>) 87
    CScriptID script_id;
    if (consensus::kvanta5p2qr::IsP2SHOutput(script, script_id)) {
        LOCK(m_mutex);

        for (const auto& [key_hash, record] : m_keys) {
            const CScript redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(key_hash);
            if (CScriptID(redeem_script) == script_id) {
                return ISMINE_SPENDABLE;
            }
        }

        for (const auto& [msig_program, record] : m_multisig) {
            const CScript redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(msig_program);
            if (CScriptID(redeem_script) == script_id) {
                return ISMINE_SPENDABLE;
            }
        }
...
std::unordered_set<CScript, SaltedSipHasher> Kvanta5P2QRScriptPubKeyMan::GetScriptPubKeys() const
{
    ...
    for (const auto& [key_hash, record] : m_keys) {
        const Kvanta5P2QRDestination dest{key_hash};

        // Native P2QR:
        //   50 20 <program>
        scripts.insert(GetScriptForDestination(dest));

        // P2SH-carried P2QR compatibility container:
        //   a9 14 HASH160(50 20 <program>) 87
        scripts.insert(consensus::kvanta5p2qr::MakeP2SHScriptPubKey(key_hash));
    }

    for (const auto& [msig_program, record] : m_multisig) {
        const Kvanta5P2QRDestination dest{msig_program};

        // Native P2QR multisig:
        //   50 20 <multisig-program>
        scripts.insert(GetScriptForDestination(dest));

        // P2SH-carried P2QR multisig compatibility container:
        //   a9 14 HASH160(50 20 <multisig-program>) 87
        scripts.insert(consensus::kvanta5p2qr::MakeP2SHScriptPubKey(msig_program));
    }
...
bool Kvanta5P2QRScriptPubKeyMan::CanProvide(const CScript& script, SignatureData& sigdata)
{
...
{
    if (record.seed.size() != MLDSA87_SEED_SIZE) return false;
    if (record.pubkey.size() != MLDSA87_PUBLIC_KEY_SIZE) return false;
...
bool Kvanta5P2QRScriptPubKeyMan::ImportSeed(Span<const unsigned char> seed, Kvanta5P2QRDestination& dest)
{
    ...
    WalletBatch batch(m_storage.GetDatabase());
    return AddKey(batch, key_hash, record);
}

bool Kvanta5P2QRScriptPubKeyMan::ExportSeed(const Kvanta5P2QRDestination& dest, std::vector<unsigned char>& seed_out) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);

    LOCK(m_mutex);
    const auto it = m_keys.find(key_hash);
    if (it == m_keys.end()) return false;

    seed_out = it->second.seed;
    return seed_out.size() == MLDSA87_SEED_SIZE;
}

bool Kvanta5P2QRScriptPubKeyMan::GetKey(const Kvanta5P2QRDestination& dest, CPQKey& key_out) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);
    ...
    if (!key_out.SetSeedBytes(it->second.seed)) return false;
    return key_out.GetHash() == key_hash;
}

bool Kvanta5P2QRScriptPubKeyMan::HaveDestination(const Kvanta5P2QRDestination& dest) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);

    LOCK(m_mutex);
    return m_keys.count(key_hash) > 0;
}

bool Kvanta5P2QRScriptPubKeyMan::GetPubKeyForDestination(const Kvanta5P2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);
...
{
    std::string error;
    if (!Kvanta5IsCanonicalP2QRMultisigPolicy(record.required, record.signer_programs, &error)) {
        WalletLogPrintf("KVANTA5_P2QR multisig policy rejected: %s\n", error);
        return false;
    }

    const uint256 expected_program = Kvanta5P2QRMultisigProgram(record.required, record.signer_programs);
    if (expected_program != program) {
        WalletLogPrintf("KVANTA5_P2QR multisig policy rejected: program mismatch\n");
        return false;
    }

    {
        LOCK(m_mutex);
        m_multisig[program] = record;
    }

    WalletBatch batch(m_storage.GetDatabase());
    return batch.WriteKvanta5P2QRMultisig(program, record);
}

bool Kvanta5P2QRScriptPubKeyMan::LoadMultisigPolicy(const uint256& program, const Kvanta5P2QRMultisigRecord& record)
{
    std::string error;
    if (!Kvanta5IsCanonicalP2QRMultisigPolicy(record.required, record.signer_programs, &error)) {
        WalletLogPrintf("KVANTA5_P2QR multisig policy load rejected: %s\n", error);
        return false;
    }

    const uint256 expected_program = Kvanta5P2QRMultisigProgram(record.required, record.signer_programs);
    if (expected_program != program) {
        WalletLogPrintf("KVANTA5_P2QR multisig policy load rejected: program mismatch\n");
        return false;
    }
...
bool Kvanta5P2QRScriptPubKeyMan::HaveMultisigProgram(const uint256& program) const
{
    LOCK(m_mutex);
    return m_multisig.count(program) > 0;
}

bool Kvanta5P2QRScriptPubKeyMan::GetMultisigRecord(const uint256& program, Kvanta5P2QRMultisigRecord& record_out) const
{
    LOCK(m_mutex);

    const auto it = m_multisig.find(program);
    if (it == m_multisig.end()) return false;

    record_out = it->second;
    return true;
}

bool Kvanta5P2QRScriptPubKeyMan::GetMultisigProgramForP2SHScriptID(const CScriptID& script_id, uint256& program_out) const
{
    LOCK(m_mutex);

    for (const auto& [program, record] : m_multisig) {
        const CScript redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(program);
        if (CScriptID(redeem_script) == script_id) {
            program_out = program;
            return true;
        }
...
std::vector<std::pair<uint256, Kvanta5P2QRMultisigRecord>>
Kvanta5P2QRScriptPubKeyMan::ListMultisigPolicies() const
{
    std::vector<std::pair<uint256, Kvanta5P2QRMultisigRecord>> result;
    result.reserve(m_multisig.size());

    for (const auto& entry : m_multisig) {
...
bool Kvanta5P2QRScriptPubKeyMan::GetKeyByPubKey(const std::vector<unsigned char>& pubkey, CPQKey& key_out) const
{
    const uint256 key_hash = Hash(pubkey);
    ...
    return key_out.GetPubKeyBytes() == pubkey;
}
...
bool Kvanta5P2QRScriptPubKeyMan::SignTransaction(
    ...
    std::map<int, bilingual_str>& input_errors) const
{
    ...
    // Native/P2SH-carried KVANTA5_P2QR v1 supports SIGHASH_DEFAULT/ALL only.
    if (sighash != SIGHASH_DEFAULT && sighash != SIGHASH_ALL) {
        return false;
    }
    ...
    CTransaction tx_const_unsigned(tx);
    ...
    for (uint32_t i = 0; i < tx.vin.size(); ++i) {
        ...
        const CTxOut& spent_output = coin_it->second.out;

        uint256 program;
        CScript p2qr_redeem_script;
        bool native_p2qr = false;
        bool wrapped_p2qr = false;

        if (consensus::kvanta5p2qr::IsOutput(spent_output.scriptPubKey, program)) {
            native_p2qr = true;
            p2qr_redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(program);
        } else {
            CScriptID spent_script_id;
            if (!consensus::kvanta5p2qr::IsP2SHOutput(spent_output.scriptPubKey, spent_script_id)) {
                continue;
            }

            // First try single-key P2SH-carried P2QR.
            {
                LOCK(m_mutex);
                for (const auto& [candidate_key_hash, record] : m_keys) {
                    const CScript candidate_redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(candidate_key_hash);
                    if (CScriptID(candidate_redeem_script) == spent_script_id) {
                        program = candidate_key_hash;
                        p2qr_redeem_script = candidate_redeem_script;
                        wrapped_p2qr = true;
                        break;
                    }
                }
            ...
            // Then try multisig P2SH-carried P2QR.
            if (!wrapped_p2qr) {
                uint256 multisig_program;
                if (GetMultisigProgramForP2SHScriptID(spent_script_id, multisig_program)) {
        ...
        const uint256 msg_hash = native_p2qr
        ...
        const bool is_multisig = GetMultisigRecord(program, multisig_record);
        ...
        if (is_multisig) {
            ...
            if (!Kvanta5IsCanonicalP2QRMultisigPolicy(
                    ...
                    &policy_error)) {
                input_errors[i] = Untranslated(strprintf(
                    "KVANTA5_P2QR multisig policy invalid: %s",
                    policy_error));
                return false;
            }

            const uint256 expected_program =
                Kvanta5P2QRMultisigProgram(multisig_record.required, multisig_record.signer_programs);
            if (expected_program != program) {
                input_errors[i] = Untranslated("KVANTA5_P2QR multisig policy program mismatch");
                return false;
            }
            ...
            signatures.reserve(multisig_record.required);
            signer_indexes.reserve(multisig_record.required);
            signing_pubkeys.reserve(multisig_record.required);

            for (uint16_t signer_index = 0;
                 signer_index < static_cast<uint16_t>(multisig_record.signer_programs.size());
                 ++signer_index) {
                if (signatures.size() >= multisig_record.required) {
                    break;
                }

                const uint256& signer_program = multisig_record.signer_programs[signer_index];
                const Kvanta5P2QRDestination signer_dest{signer_program};

                std::vector<unsigned char> pubkey;
                if (!GetPubKeyForDestination(signer_dest, pubkey)) {
                    continue;
                }
                ...
                if (derived_signer_program != signer_program) {
                    input_errors[i] = Untranslated(
                        "KVANTA5_P2QR multisig local pubkey does not match signer address policy");
                    return false;
                }

                CPQKey key;
                if (!GetKeyByPubKey(pubkey, key)) {
                    continue;
                }

                std::vector<unsigned char> signature;
                if (!key.Sign(Span<const unsigned char>(msg_hash.begin(), msg_hash.size()), signature)) {
                    input_errors[i] = Untranslated("KVANTA5_P2QR multisig ML-DSA-87 signing failed");
                    return false;
                }

                if (signature.empty()) {
                    input_errors[i] = Untranslated("KVANTA5_P2QR multisig ML-DSA-87 signing returned empty signature");
                    return false;
                }

                signatures.push_back(std::move(signature));
                signer_indexes.push_back(signer_index);
                signing_pubkeys.push_back(std::move(pubkey));
            }

            if (signatures.size() < multisig_record.required) {
                input_errors[i] = Untranslated(strprintf(
                    "KVANTA5_P2QR multisig private keys not found: have %u, need %u",
                    static_cast<unsigned int>(signatures.size()),
                    static_cast<unsigned int>(multisig_record.required)));
                return false;
            }

            // Address-policy multisig envelope:
            // <sig_0> ... <sig_m-1>
            // <idx_0> ... <idx_m-1>
            // <signing_pubkey_0> ... <signing_pubkey_m-1>
            // <signer_program_0> ... <signer_program_n-1>
            // <m>
            // <n>
            ...
            for (const uint16_t signer_index : signer_indexes) {
                txin.scriptSig << Kvanta5P2QRUInt16Push(signer_index);
            }
            ...
            for (const uint256& signer_program : multisig_record.signer_programs) {
                txin.scriptSig << std::vector<unsigned char>(signer_program.begin(), signer_program.end());
            }

            txin.scriptSig << Kvanta5P2QRUInt16Push(multisig_record.required);
            txin.scriptSig << Kvanta5P2QRUInt16Push(static_cast<uint16_t>(multisig_record.signer_programs.size()));

            if (wrapped_p2qr) {
                txin.scriptSig << std::vector<unsigned char>(p2qr_redeem_script.begin(), p2qr_redeem_script.end());
            }
        } else {
            ...
            CPQKey key;
            if (!GetKey(dest, key)) {
                input_errors[i] = Untranslated("KVANTA5_P2QR private key not found");
                return false;
            }

            std::vector<unsigned char> signature;
            if (!key.Sign(Span<const unsigned char>(msg_hash.begin(), msg_hash.size()), signature)) {
                input_errors[i] = Untranslated("KVANTA5_P2QR ML-DSA-87 signing failed");
                return false;
            }

            if (signature.empty()) {
                input_errors[i] = Untranslated("KVANTA5_P2QR ML-DSA-87 signing returned empty signature");
                return false;
            }

            if (native_p2qr) {
                txin.scriptSig << signature << key.GetPubKeyBytes();
            } else {
                txin.scriptSig
                    << signature
                    << key.GetPubKeyBytes()
                    << std::vector<unsigned char>(p2qr_redeem_script.begin(), p2qr_redeem_script.end());
            }
        }
    ...
    // This SPKM's responsibility is only native/P2SH-carried KVANTA5_P2QR inputs.
    // Mixed-input transactions may also contain legacy, SegWit, Taproot, etc.,
    // which are owned and signed by other ScriptPubKeyMans.
    //
    // Therefore, after self-verifying every P2QR input we actually signed, report
    // whether this SPKM made successful P2QR signing progress. Do not require the
    // whole transaction to be all-P2QR.
    return signed_any;
}

bool Kvanta5P2QRScriptPubKeyMan::GetDestinationForP2SHScriptID(const CScriptID& script_id, Kvanta5P2QRDestination& dest) const
{
    LOCK(m_mutex);
    ...
    for (const auto& [key_hash, record] : m_keys) {
        const CScript redeem_script = consensus::kvanta5p2qr::MakeRedeemScript(key_hash);
        if (CScriptID(redeem_script) == script_id) {
            dest = Kvanta5P2QRDestination{key_hash};
            return true;
        }
    }
