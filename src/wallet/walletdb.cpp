...
{
    return WriteIC(std::make_pair(DBKeys::KVANTA5_P2QR_MULTISIG, program), record, false);
}
...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    LoadResult kvanta5_p2qr_key_res = LoadRecords(pwallet, batch, DBKeys::KVANTA5_P2QR_KEY,
        ...
        if (!pwallet->LoadKvanta5P2QRKey(key_hash, record)) {
            strErr = "Error reading wallet database: CWallet::LoadKvanta5P2QRKey failed";
    ...
    LoadResult kvanta5_p2qr_multisig_res = LoadRecords(pwallet, batch, DBKeys::KVANTA5_P2QR_MULTISIG,
        ...
        if (!pwallet->LoadKvanta5P2QRMultisig(program, record)) {
            strErr = "Error reading wallet database: CWallet::LoadKvanta5P2QRMultisig failed";
            return DBErrors::NONCRITICAL_ERROR;
        }
    ...
    LoadResult keymeta_res = LoadRecords(pwallet, batch, DBKeys::KEYMETA,
        ...
        if (keyMeta.nVersion >= CKeyMetadata::VERSION_WITH_HDDATA && !keyMeta.hd_seed_id.IsNull() && keyMeta.hdKeypath.size() > 0) {
            ...
            // See https://github.com/kvanta5/kvanta5/pull/12924
            bool internal = false;
            uint32_t index = 0;
