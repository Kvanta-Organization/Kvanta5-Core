...
#ifndef KVANTA5_WALLET_SCRIPTPUBKEYMAN_H
#define KVANTA5_WALLET_SCRIPTPUBKEYMAN_H
 ...
 * change.
 *
 * The Kvanta5 Core wallet was originally a collection of unrelated private
...
class Kvanta5P2QRScriptPubKeyMan : public ScriptPubKeyMan
{
private:
    ...
    {
        uint256 key_hash;
        std::copy(dest.begin(), dest.end(), key_hash.begin());
        return key_hash;
    }

public:
    explicit Kvanta5P2QRScriptPubKeyMan(WalletStorage& storage) : ScriptPubKeyMan(storage) {}

    util::Result<CTxDestination> GetNewDestination(const OutputType type) override;
    isminetype IsMine(const CScript& script) const override;
    bool GetDestinationForP2SHScriptID(const CScriptID& script_id, Kvanta5P2QRDestination& dest) const;
    bool CanGetAddresses(bool internal = false) const override { return !internal; }
    bool HavePrivateKeys() const override { return true; }
    uint256 GetID() const override { return Hash(std::string{"kv5-p2qr-spkm"}); }
    ...
    bool GetKey(const Kvanta5P2QRDestination& dest, CPQKey& key_out) const;
    bool GetPubKeyForDestination(const Kvanta5P2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const;
    bool HaveDestination(const Kvanta5P2QRDestination& dest) const;

    bool AddMultisigPolicy(const uint256& program, const Kvanta5P2QRMultisigRecord& record);
    bool LoadMultisigPolicy(const uint256& program, const Kvanta5P2QRMultisigRecord& record);
    bool HaveMultisigProgram(const uint256& program) const;
    bool GetMultisigRecord(const uint256& program, Kvanta5P2QRMultisigRecord& record_out) const;
    bool GetMultisigProgramForP2SHScriptID(const CScriptID& script_id, uint256& program_out) const;
    std::vector<std::pair<uint256, Kvanta5P2QRMultisigRecord>> ListMultisigPolicies() const;
    bool GetKeyByPubKey(const std::vector<unsigned char>& pubkey, CPQKey& key_out) const;
};
