...
static void SetupKvanta5TxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " kvanta5-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The kvanta5-tx tool is used for creating and modifying kvanta5 transactions.\n\n"
                "kvanta5-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded kvanta5 transaction, or with \"-create [commands]\" to create a hex-encoded kvanta5 transaction.\n"
                "\n"
                "Usage: kvanta5-tx [options] <hex-tx> [commands]\n"
                "or:    kvanta5-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded kvanta5 transaction
