This directory contains integration tests that test kvanta5d and its
...
kvanta5d and kvanta5-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (kvanta5-util, kvanta5-tx, ...).
...
Before tests can be run locally, Kvanta5 Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the kvanta5d nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another kvanta5d
process running on the system (perhaps from a previous test which hasn't successfully
killed all its kvanta5d nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other kvanta5d processes are running.

On linux, the test framework will warn if there is another
kvanta5d process running when the tests are started.

If there are zombie kvanta5d processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
kvanta5d processes running on the system, so should not be used if any non-test
kvanta5d processes are being run.**
...
sure kvanta5d processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and kvanta5d `debug.log`s will all be dumped
...
`test_framework.log` and kvanta5d `debug.log`s can be combined into a single
...
call methods that interact with the kvanta5d nodes-under-test.

If further introspection of the kvanta5d instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/kvanta5d.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/kvanta5d <pid>
```
