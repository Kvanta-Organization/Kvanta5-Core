...
class InitTest(Kvanta5TestFramework):
    """
    ...
    def init_stress_test(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # kvanta5d to perform any shutdown logic.
    ...
    def init_pid_test(self):
        KVANTA5_PID_FILENAME_CUSTOM = "my_fancy_kvanta5_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = KVANTA5_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / KVANTA5_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / KVANTA5_PID_FILENAME_CUSTOM
