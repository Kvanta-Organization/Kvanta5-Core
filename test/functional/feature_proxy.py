...
"""Test kvanta5d with different proxy configuration.

Test plan:
- Start kvanta5d's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on kvanta5d side:
...
class ProxyTest(Kvanta5TestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: kvanta5d's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
