...
"""Check that it's not possible to start a second kvanta5d instance using the same datadir or wallet."""
import random
import string

from test_framework.test_framework import Kvanta5TestFramework
from test_framework.test_node import (
    KVANTA5_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(Kvanta5TestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second kvanta5d instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second kvanta5d instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second kvanta5d using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second kvanta5d instance shutdown
        pid_file = datadir / KVANTA5_PID_FILENAME_DEFAULT
