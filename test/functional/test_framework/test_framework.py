...
TMPDIR_PREFIX = "kvanta5_func_test_"
...
class Kvanta5TestMetaClass(type):
    """Metaclass for Kvanta5TestFramework.

    Ensures that any attempt to register a subclass of `Kvanta5TestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'Kvanta5TestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("Kvanta5TestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("Kvanta5TestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class Kvanta5TestFramework(metaclass=Kvanta5TestMetaClass):
    """Base class for a kvanta5 test script.

    Individual kvanta5 test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave kvanta5ds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use kvanta5-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "kvanta5d": ("kvanta5d", "KVANTA5D"),
            "kvanta5-cli": ("kvanta5cli", "KVANTA5CLI"),
            "kvanta5-util": ("kvanta5util", "KVANTA5UTIL"),
            "kvanta5-wallet": ("kvanta5wallet", "KVANTA5WALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("Kvanta5RPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'kvanta5d', self.options.kvanta5d) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'kvanta5-cli', self.options.kvanta5cli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a kvanta5d test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple kvanta5d test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the kvanta5d
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as kvanta5d's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    kvanta5d=self.options.kvanta5d,
                    kvanta5_cli=self.options.kvanta5cli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in kvanta5.conf
    ...
    def skip_if_no_kvanta5d_tracepoints(self):
        """Skip the running test if kvanta5d has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("kvanta5d has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_kvanta5d_zmq(self):
        """Skip the running test if kvanta5d has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("kvanta5d has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if kvanta5-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("kvanta5-wallet has not been compiled")

    def skip_if_no_kvanta5_util(self):
        """Skip the running test if kvanta5-util has not been compiled."""
        if not self.is_kvanta5_util_compiled():
            raise SkipTest("kvanta5-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if kvanta5-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("kvanta5-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether kvanta5-cli was compiled."""
    ...
    def is_kvanta5_util_compiled(self):
        """Checks whether kvanta5-util was compiled."""
