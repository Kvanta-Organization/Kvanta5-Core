...
`test/functional/test_framework/test_framework.py:Kvanta5TestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_kvanta5_cli.py --legacy-wallet',
    'interface_kvanta5_cli.py --descriptors',
...
def main():
    ...
    enable_kvanta5d = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_kvanta5d:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if kvanta5d is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "kvanta5d"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a kvanta5d process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Kvanta5 functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/kvanta5/kvanta5/pull/27561.
...
class RPCCoverage():
    ...
    commands per `kvanta5-cli help` (`rpc_interface.txt`).
