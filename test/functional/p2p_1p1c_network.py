...
class PackageRelayTest(Kvanta5TestFramework):
    ...
    def raise_network_minfee(self):
        ...
        for node in self.nodes:
            assert_equal(node.getmempoolinfo()['minrelaytxfee'], FEERATE_1QUARK_VB)
            assert_greater_than(node.getmempoolinfo()['mempoolminfee'], FEERATE_1QUARK_VB)

    def create_basic_1p1c(self, wallet):
        low_fee_parent = wallet.create_self_transfer(fee_rate=FEERATE_1QUARK_VB, confirmed_only=True)
    ...
    def create_package_2outs(self, wallet):
        ...
        # Target 1quark/vB so the number of quarks is equal to the vsize.
    ...
    def create_package_2p1c(self, wallet):
        parent1 = wallet.create_self_transfer(fee_rate=FEERATE_1QUARK_VB*10, confirmed_only=True)
        parent2 = wallet.create_self_transfer(fee_rate=FEERATE_1QUARK_VB*20, confirmed_only=True)
    ...
    def create_packages(self):
        # 1: Basic 1-parent-1-child package, parent 1quark/vB, child 999quark/vB
