...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing kvanta5-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='kvanta5-cli', help='kvanta5-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Kvanta5 address to which the faucet should send')
...
parser.add_argument('kvanta5_cli_args', nargs='*', help='Arguments to pass on to kvanta5-cli (default: -signet)')

args = parser.parse_args()

if args.kvanta5_cli_args == []:
    args.kvanta5_cli_args = ['-signet']


def kvanta5_cli(rpc_command_and_params):
    argv = [args.cmd] + args.kvanta5_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = kvanta5_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = kvanta5_cli(['getnewaddress', 'faucet', 'bech32'])
