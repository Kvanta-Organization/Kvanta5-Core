...
Tracing (USDT) support for Kvanta5 Core on Linux using. For more information on
USDT support in Kvanta5 Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `kvanta5d` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `kvanta5d` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Kvanta5 Core USDT documentation] on how to list available tracepoints in your
`kvanta5d` binary.

[Kvanta5 Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof kvanta5d)
```
...
Kvanta5 P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof kvanta5d)
```
...
In a different terminal, starting Kvanta5 Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/kvanta5d -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof kvanta5d)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof kvanta5d)
```
