...
"""Script for verifying Kvanta5 Core release binaries.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/kvanta5-core/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Kvanta5 community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"kvanta5_verify_binaries.{args.version}"
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the kvanta5 release to download; of the format '
