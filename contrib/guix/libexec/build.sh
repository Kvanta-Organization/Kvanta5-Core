...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/kvanta5-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Kvanta5 Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Kvanta5 Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example kvanta5.conf file. if contrib/devtools/gen-kvanta5-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/kvanta5.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/Kvanta5-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
